...
**Re-verified 2026-09-13 on `origin/main` `f737dd4c` (after Toman Phase C, Alembic `0115`/`0116`):**
every function named below still exists with the described behaviour; amounts, the Currency section
and test numbers were moved to the Toman 1:1 scale; Mini App line references refreshed. No design
point changed.
...
**Today (evidence on `1443fe79`, re-checked on `f737dd4c`), partner 433 at 3000 bps, tariff 9
«روزانه» `daily_price_kopeks` = 10,000 (Toman 1:1 since Phase C; 10,000 Toman/day):**
...
**Currency:** Toman 1:1 end to end (Phase C). Every price and balance in `PricingEngine` is the
same Toman number; charges compare with `user_can_afford`. The cabinet response fields keep going
through `wire_catalog_kopeks` exactly as today (the only x100, `app/utils/wire_scale.py`); Mini App
model fields stay 1:1 as today. No new `/ 100` or `* 100`. Phase C-2: out of scope, not widened.
...
- Files: `app/services/pricing_engine.py` (`daily_group_price`, ~line 128). Tests:
  `tests/services/test_wholesale_pricing.py` (unit), `tests/cabinet/test_daily_price_display.py`
  (extend: for a wholesale partner the cabinet's `daily_price_kopeks` is the wire form of 7,000 and
  `original_daily_price_kopeks` of 10,000). The recurring charge and every other caller already
  price through `daily_group_price` — pinned by the AST guards in
  `tests/services/test_daily_charge_fork_sites.py` and `test_daily_price_parity_surfaces.py`.
...
- Test first (logic): approved 3000 bps, 10,000 → (7_000, 30); approved 175 bps → floor price, pct 2;
...
- Test first: partner with an active 20% offer switching periodic → tariff 9 (10,000) costs 7,000
  (not 10,000 − group − 20%); daily → periodic costs the wholesale min-period price; a periodic upgrade
...
- Files: `app/cabinet/routes/promo.py` (`get_active_discount`, ~line 145). Mini App parity: the
  ...
  Mini App lines as of `f737dd4c`: `get_subscription_details` reports `promo_offer_discount_percent`
  (~line 3236 / 3484); `_build_tariff_model` reads the offer itself through
  `get_user_active_promo_discount_percent` (~6398 periods, ~6459 daily) — its wholesale branch is
  Task 4.

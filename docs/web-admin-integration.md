...
- Если `reward_enabled=true`, сумма вознаграждения (`reward_amount_toman`) должна быть положительной. При `false` значение автоматически сбрасывается в `0`.

...
async def run_universal_migration():
    """
    Universal migration is deprecated in favor of Alembic migrations.
    
    For dev/staging environments, use Alembic migrations instead:
        alembic upgrade head
    
    This function now only syncs PostgreSQL sequences (if needed)
    and returns immediately.
    """
    logger.info("=== UNIVERSAL MIGRATION (DEPRECATED) ===")
    logger.info("ℹ️ Using Alembic migrations instead. Run: alembic upgrade head")

    try:
        db_type = await get_database_type()
        logger.info(f"Database type: {db_type}")

        # Only sync PostgreSQL sequences (safe operation)
        if db_type == "postgresql":
        ...
        # Skip all other migrations - use Alembic instead
        logger.info("✅ Universal migration skipped (using Alembic)")

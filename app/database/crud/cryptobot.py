...
async def get_cryptobot_payment_by_invoice_id(db: AsyncSession, invoice_id: str) -> Optional[CryptoBotPayment]:
...
async def get_cryptobot_payment_by_id(db: AsyncSession, payment_id: int) -> Optional[CryptoBotPayment]:
    result = await db.execute(
...
async def update_cryptobot_payment_status(
    db: AsyncSession, invoice_id: str, status: str, paid_at: Optional[datetime] = None

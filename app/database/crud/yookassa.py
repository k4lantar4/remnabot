...
async def get_yookassa_payment_by_id(db: AsyncSession, yookassa_payment_id: str) -> Optional[YooKassaPayment]:
...
async def get_yookassa_payment_by_local_id(db: AsyncSession, local_id: int) -> Optional[YooKassaPayment]:
    result = await db.execute(
...
async def get_pending_yookassa_payments(
    db: AsyncSession, user_id: Optional[int] = None, limit: int = 100
) -> List[YooKassaPayment]:
...
async def get_yookassa_payments_stats(db: AsyncSession, user_id: Optional[int] = None) -> dict:
    from sqlalchemy import func, case

    query = select(
        func.count(YooKassaPayment.id).label("total_payments"),
        func.sum(YooKassaPayment.amount_toman).label("total_amount_toman"),
        func.sum(case((YooKassaPayment.status == "succeeded", YooKassaPayment.amount_toman), else_=0)).label(
            "succeeded_amount_toman"
        ),

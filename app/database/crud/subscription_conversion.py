...
async def create_subscription_conversion(
    ...
    logger.info(
        f"✅ Conversion record created for user {user_id}: {trial_duration_days} days → {first_paid_period_days} days for {first_payment_amount_toman / 100} Toman"
    )

    return conversion


async def get_conversion_by_user_id(db: AsyncSession, user_id: int) -> Optional[SubscriptionConversion]:
...
async def get_conversion_statistics(db: AsyncSession) -> dict:
    ...
    avg_first_payment_result = await db.execute(select(func.avg(SubscriptionConversion.first_payment_amount_toman)))

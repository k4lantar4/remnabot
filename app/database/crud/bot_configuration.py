...
async def get_all_configurations(
    ...
    result = await db.execute(
        select(BotConfiguration)
        .where(BotConfiguration.bot_id == bot_id)
        .order_by(BotConfiguration.config_key)
    )
    return list(result.scalars().all())


async def get_all_configurations_dict(
    db: AsyncSession,
    bot_id: int
    ...
    return {
        config.config_key: (
            config.config_value if isinstance(config.config_value, dict) else {}
        )

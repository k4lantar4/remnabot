...
async def create_server_squad(
    ...
    if len(promo_groups) != len(normalized_group_ids):
        logger.warning("Not all promo groups found when creating server %s", display_name)
...
async def get_server_squad_by_id(db: AsyncSession, server_id: int) -> Optional[ServerSquad]:
    result = await db.execute(
...
async def update_server_squad(db: AsyncSession, server_id: int, **updates) -> Optional[ServerSquad]:
    valid_fields = {
        "display_name",
        "original_name",
        "country_code",
...
async def get_server_statistics(db: AsyncSession) -> dict:
    ...
    revenue_result = await db.execute(select(func.coalesce(func.sum(SubscriptionServer.paid_price_toman), 0)))

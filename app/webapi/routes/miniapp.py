...
async def _build_tariff_model(
    ...
    if is_daily and daily_price_kopeks > 0 and wholesale_pct:
...
async def purchase_tariff_endpoint(
    ...
    # выберет create; существующая — update (сквады шлёт по умолчанию).
    # При покупке тарифа ВСЕГДА сбрасываем трафик.
    service = SubscriptionService()

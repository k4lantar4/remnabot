...
async def start_cryptobot_payment(callback: types.CallbackQuery, db_user: User, state: FSMContext):
    ...
    if not settings.is_cryptobot_enabled():
        await callback.answer(
            texts.t("CRYPTOBOT_UNAVAILABLE", "❌ Cryptocurrency payment temporarily unavailable"), show_alert=True
        )
        return
    ...
    try:
        ...
        rate_text = texts.t("CRYPTOBOT_RATE_CURRENT", "💱 Current rate: 1 USD = {rate}").format(
            rate=f"{current_rate:.2f}  Toman"
        )
    except Exception as e:
        logger.warning(f"Failed to get exchange rate: {e}")
        current_rate = 95.0
        rate_text = texts.t("CRYPTOBOT_RATE_APPROX", "💱 Rate: 1 USD ≈ {rate}").format(
            rate=f"{current_rate:.0f}  Toman"
        )
...
async def process_cryptobot_payment_amount(
    ...
    try:
        ...
        logger.info(
            f"Created CryptoBot payment for user {db_user.telegram_id}: "
...
@error_handler
async def check_cryptobot_payment_status(callback: types.CallbackQuery, db: AsyncSession, db_user: User = None):

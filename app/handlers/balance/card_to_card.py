...
async def start_card_to_card_payment(
    ...
    if not card:
        await callback.answer(
            texts.t("CARD_TO_CARD_NO_CARD", "❌ No payment card available. Please contact support."), show_alert=True
        )
        return
    ...
    if not amount_toman or amount_toman <= 0:
        await callback.answer(texts.t("CARD_TO_CARD_NO_AMOUNT", "❌ Please select an amount first"), show_alert=True)
        return
...
You can send an image, text, or both.""",
        ...
        card_number=card.card_number, card_holder=card.card_holder_name, amount=f"{amount_rub:,.2f}".replace(",", " ")
    )

    await callback.message.edit_text(card_info, reply_markup=get_back_keyboard(db_user.language), parse_mode="HTML")

    # Save card_id and amount in state
    await state.update_data(card_id=card.id, amount_toman=amount_toman, payment_method="card_to_card")
...
async def handle_card_to_card_receipt(
    ...
    if bot_id is None:
        bot_id = getattr(db_user, "bot_id", None)
        if bot_id is None:
            await message.answer(texts.t("PAYMENT_ERROR", "❌ Payment error occurred"))
            return
    ...
    if not card_id or not amount_toman:
        await message.answer(texts.t("CARD_TO_CARD_SESSION_EXPIRED", "❌ Session expired. Please start over."))
        await state.clear()
        return
    ...
    if message.photo:
        ...
        await message.answer(texts.t("CARD_TO_CARD_INVALID_RECEIPT", "❌ Please send an image or text receipt"))
        return
...
After review, you will be notified of the result.""",
        ...
        await message.answer(texts.t("PAYMENT_ERROR", "❌ Payment error occurred. Please try again."))


async def send_admin_notification(db: AsyncSession, bot_id: int, payment, bot: Bot):
...
💳 <b>Card:</b> {payment.card.card_number if payment.card else "N/A"}"""
        ...
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(text="✅ Approve", callback_data=f"approve_card_payment:{payment.id}"),
                    types.InlineKeyboardButton(text="❌ Reject", callback_data=f"reject_card_payment:{payment.id}"),
                ],
                [types.InlineKeyboardButton(text="📋 Details", callback_data=f"card_payment_details:{payment.id}")],
            ]
        )
...
async def handle_payment_approval(
    ...
    if not settings.is_admin(db_user.telegram_id):
        await callback.answer(texts.t("ACCESS_DENIED", "❌ Access denied"), show_alert=True)
        return
    ...
    if payment.status != "pending":
        await callback.answer(
            texts.t("PAYMENT_ALREADY_REVIEWED", "❌ This payment has already been reviewed"), show_alert=True
        )
        return
    ...
    try:
        ...
        await update_payment_status(
            db=db, payment_id=payment_id, status="approved", transaction_id=transaction.id, bot_id=bot_id
        )
...
Your balance has been updated.""",
                        ...
                        tracking=payment.tracking_number, amount=f"{payment.amount_toman / 100:,.2f}".replace(",", " ")
                    ),
                    parse_mode="HTML",
                )
        ...
        await callback.answer(texts.t("PAYMENT_ERROR", "❌ Error processing payment"), show_alert=True)
...
Please contact support for more information.""",
        ...
        try:
            await callback.message.edit_text(
                callback.message.text + "\n\n❌ <b>Rejected</b>", reply_markup=None, parse_mode="HTML"
            )
        except Exception:
            pass
...
def register_card_to_card_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(start_card_to_card_payment, F.data == "payment_card_to_card")

    dp.message.register(handle_card_to_card_receipt, StateFilter(BalanceStates.waiting_for_card_to_card_receipt))

    dp.callback_query.register(handle_payment_approval, F.data.startswith("approve_card_payment:"))

    dp.callback_query.register(handle_payment_rejection, F.data.startswith("reject_card_payment:"))

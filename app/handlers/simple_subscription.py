...
async def start_simple_subscription_purchase(
    ...
    if show_devices:
        devices_text = texts.t("SIMPLE_SUBSCRIPTION_DEVICES", "📱 Devices: {count}").format(
...
def _get_simple_subscription_payment_keyboard(language: str) -> types.InlineKeyboardMarkup:
    ...
    if settings.is_wata_enabled():
        keyboard.append(
            [
                types.InlineKeyboardButton(
                    text=texts.t("PAYMENT_METHOD_WATA_BUTTON", "💳 WATA"), callback_data="simple_subscription_wata"
                )
            ]
        )
...
async def handle_simple_subscription_pay_with_balance(
    ...
    try:
        ...
        period_text = texts.t("SUBSCRIPTION_PERIOD_DAYS", "{days} days").format(days=subscription_params["period_days"])
        payment_description = texts.t(
            "SIMPLE_SUBSCRIPTION_PAYMENT_DESCRIPTION", "Subscription payment for {period}"
        ...
        if not subscription:
            ...
            period_text = texts.t("SUBSCRIPTION_PERIOD_DAYS", "{days} days").format(
                days=subscription_params["period_days"]
            )
            refund_description = texts.t(
                "SIMPLE_SUBSCRIPTION_REFUND_DESCRIPTION", "Refund for failed subscription for {period}"
            ).format(period=period_text)
            ...
            await callback.answer(
                texts.t(
                    "SIMPLE_SUBSCRIPTION_CREATION_ERROR",
                    "❌ Error creating subscription. Funds have been refunded to balance.",
                ),
                show_alert=True,
            )
            return
        ...
        try:
            ...
            logger.error(
                "Error syncing subscription with RemnaWave for user %s: %s", db_user.id, sync_error, exc_info=True
            )
        ...
        success_lines.extend(
            [
                ...
                texts.t("SIMPLE_SUBSCRIPTION_SERVER", "🌍 Server: {server}").format(server=server_label),
                "",
                texts.t("SIMPLE_SUBSCRIPTION_DEDUCTED", "💰 Deducted from balance: {amount}").format(
        ...
        keyboard_rows.append(
            [
                types.InlineKeyboardButton(
                    text=texts.t("BACK_TO_MAIN_MENU_BUTTON", "🏠 Main menu"), callback_data="back_to_menu"
                )
            ]
        )
    ...
    message_lines.extend(
        [
            ...
            (
                ...
                if can_pay_from_balance
                else texts.t("SIMPLE_SUBSCRIPTION_PAYMENT_OPTIONS_INSUFFICIENT", "Choose a suitable payment method:")
            ),
        ]
    )
...
async def handle_simple_subscription_payment_method(
    ...
    try:
        ...
        if payment_method == "stars":
            ...
            invoice_title = texts.t("SIMPLE_SUBSCRIPTION_INVOICE_TITLE", "Subscription for {period}").format(
                period=period_text
            )
            invoice_description = texts.t(
                "SIMPLE_SUBSCRIPTION_INVOICE_DESCRIPTION",
                "Simple subscription purchase\nPeriod: {period}\nDevices: {devices}\nTraffic: {traffic}",
            ).format(period=period_text, devices=subscription_params["device_limit"], traffic=stars_traffic_label)
            ...
            if not settings.is_yookassa_enabled():
                await callback.answer(
                    texts.t("YOOKASSA_DISABLED", "❌ Payment via YooKassa is temporarily unavailable"), show_alert=True
                )
                return

            if payment_method == "yookassa_sbp" and not settings.YOOKASSA_SBP_ENABLED:
                await callback.answer(
                    texts.t("YOOKASSA_SBP_DISABLED", "❌ Payment via SBP is temporarily unavailable"), show_alert=True
                )
                return
            ...
            if not order:
                await callback.answer(
                    texts.t("SIMPLE_SUBSCRIPTION_ORDER_CREATION_ERROR", "❌ Error creating order"), show_alert=True
                )
                return
            ...
            if not payment_result:
                await callback.answer(
                    texts.t("SIMPLE_SUBSCRIPTION_PAYMENT_CREATION_ERROR", "❌ Error creating payment"), show_alert=True
                )
                return
            ...
            if not confirmation_url and not qr_confirmation_data:
                await callback.answer(
                    texts.t("SIMPLE_SUBSCRIPTION_PAYMENT_DATA_ERROR", "❌ Error getting payment data"), show_alert=True
                )
                return
            ...
            if confirmation_url:
                keyboard_buttons.append(
                    [
                        types.InlineKeyboardButton(
                            text=texts.t("GO_TO_PAYMENT_BUTTON", "🔗 Go to payment"), url=confirmation_url
                        )
                    ]
                )
            else:
                ...
                keyboard_buttons.append(
                    [
                        types.InlineKeyboardButton(
                            text=texts.t("PAY_IN_BANK_APP_BUTTON", "📱 Pay in bank app"), callback_data="temp_disabled"
                        )
                    ]
                )
            ...
            message_lines.extend(
                [
                    texts.t("SIMPLE_SUBSCRIPTION_TRAFFIC", "📊 Traffic: {traffic}").format(
                        traffic=yookassa_traffic_label
                    ),
                    texts.t("SIMPLE_SUBSCRIPTION_AMOUNT", "💰 Amount: {amount}").format(
                        amount=settings.format_price(price_toman)
                    ),
                    texts.t("SIMPLE_SUBSCRIPTION_PAYMENT_ID", "🆔 Payment ID: {id}").format(
            ...
            if amount_rubles < 100 or amount_rubles > 100000:
                await callback.answer(
                    ...
                    ).format(min=settings.format_price(10000), max=settings.format_price(10000000)),
                    show_alert=True,
                )
                return
            ...
            if amount_usd < 1:
                await callback.answer(
                    texts.t(
                        "CRYPTOBOT_MIN_AMOUNT_ERROR", "❌ Minimum amount for CryptoBot payment is approximately 1 USD"
                    ),
                    show_alert=True,
                )
                return
            ...
            message_text = message_template.format(
                ...
                rate=f"{usd_rate:.2f}",
                currency=settings.format_price(100).split()[-1]
                if settings.format_price(100)
                else texts.t("CURRENCY_RUB", "RUB"),
            ...
            if not settings.is_heleket_enabled():
                await callback.answer(
                    texts.t("HELEKET_DISABLED", "❌ Payment via Heleket is temporarily unavailable"), show_alert=True
                )
                return
            ...
            if amount_rubles < 100 or amount_rubles > 100000:
                await callback.answer(
                    texts.t(
                        "HELEKET_AMOUNT_RANGE_ERROR", "❌ Amount must be between {min} and {max} for Heleket payment"
                    ).format(min=settings.format_price(10000), max=settings.format_price(10000000)),
                    show_alert=True,
                )
                return
            ...
            if not payment_url:
                await callback.answer(
                    texts.t(
                        "HELEKET_PAYMENT_URL_ERROR", "❌ Failed to get Heleket payment link. Please contact support."
                    ),
                    show_alert=True,
                )
                return
            ...
            if payer_amount and payer_currency:
                ...
                try:
                    ...
                    if payer_amount_float > 0:
                        ...
                        currency_symbol = (
                            settings.format_price(100).split()[-1]
                            if settings.format_price(100)
                            else texts.t("CURRENCY_RUB", "RUB")
                        )
                        message_lines.append(
                            texts.t("SIMPLE_SUBSCRIPTION_EXCHANGE_RATE", "💱 Rate: 1 {currency} ≈ {rate} {rub}").format(
                                currency=payer_currency, rate=f"{rub_per_currency:.2f}", rub=currency_symbol
                            )
                        )
            ...
            if not settings.is_mulenpay_enabled():
                await callback.answer(
                    texts.t("MULENPAY_DISABLED", "❌ Payment via {name} is temporarily unavailable").format(
                        name=mulenpay_name
                    ),
                    show_alert=True,
                )
            ...
            if not settings.is_pal24_enabled():
                await callback.answer(
                    texts.t("PAL24_DISABLED", "❌ Payment via PayPalych is temporarily unavailable"), show_alert=True
                )
                return
            ...
            if not settings.is_wata_enabled():
                await callback.answer(
                    texts.t("WATA_DISABLED", "❌ Payment via WATA is temporarily unavailable"), show_alert=True
                )
                return
            if price_toman < settings.WATA_MIN_AMOUNT_TOMAN or price_toman > settings.WATA_MAX_AMOUNT_TOMAN:
                await callback.answer(
                    texts.t(
                        "WATA_AMOUNT_RANGE_ERROR", "❌ Amount for WATA must be between {min_amount} and {max_amount}"
                    ).format(
            ...
            await callback.answer(
                texts.t("SIMPLE_SUBSCRIPTION_UNKNOWN_PAYMENT_METHOD", "❌ Unknown payment method"), show_alert=True
            )
    ...
    try:
        ...
        if payment.is_paid:
            ...
            message_lines += [
                "",
                texts.t(
                    "PAYMENT_NOT_COMPLETE_CORRECTLY", "❌ Payment not completed correctly. Contact {support}"
        ...
        db_user = getattr(callback, "db_user", None)
        texts = get_texts(db_user.language if db_user else settings.DEFAULT_LANGUAGE)
        await callback.answer(texts.t("PAYMENT_STATUS_CHECK_ERROR", "❌ Error checking status"), show_alert=True)
...
async def check_simple_mulenpay_payment_status(
    ...
    try:
        ...
        await callback.answer(texts.t("PAYMENT_ID_INVALID", "❌ Invalid payment identifier"), show_alert=True)
        return
    ...
    message_lines = [
        ...
        texts.t("PAYMENT_AMOUNT", "💰 Amount: {amount}").format(amount=settings.format_price(payment.amount_toman)),
        texts.t("PAYMENT_STATUS", "📊 Status: {emoji} {status}").format(emoji=emoji, status=status_text),
        texts.t("PAYMENT_CREATED", "📅 Created: {date}").format(
    ...
    if payment.is_paid:
        ...
        message_lines.append(
            "\n"
            + texts.t(
                "PAYMENT_NOT_COMPLETE_YET", "⏳ Payment not yet completed. Complete payment and check status later."
            )
        )
...
async def check_simple_cryptobot_payment_status(
    ...
    message_lines = [
        ...
        texts.t("CRYPTOBOT_PAYMENT_AMOUNT", "💰 Amount: {amount} {asset}").format(
    ...
    if payment.status == "paid":
        ...
        message_lines.append(
            ...
            + texts.t(
                ...
                "⏳ Payment still awaiting confirmation. Pay the bill and check status later.",
            )
        )
...
async def check_simple_heleket_payment_status(
    ...
    if payment.is_paid:
        ...
        message_lines.append(
            "\n"
            + texts.t(
                "PAYMENT_STILL_PROCESSING", "⏳ Payment is still processing. Complete payment and check status later."
            )
        )

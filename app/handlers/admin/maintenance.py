...
async def show_maintenance_panel(callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    status_text = (
        texts.get_text("ADMIN_MAINTENANCE_STATUS_ENABLED", "Enabled")
        if status_info["is_active"]
        else texts.get_text("ADMIN_MAINTENANCE_STATUS_DISABLED", "Disabled")
    )

    api_emoji = "✅" if status_info["api_status"] else "❌"
    api_text = (
        texts.get_text("ADMIN_MAINTENANCE_API_AVAILABLE", "Available")
        if status_info["api_status"]
        else texts.get_text("ADMIN_MAINTENANCE_API_UNAVAILABLE", "Unavailable")
    )

    monitoring_emoji = "🔄" if status_info["monitoring_active"] else "⏹️"
    monitoring_text = (
        texts.get_text("ADMIN_MAINTENANCE_MONITORING_RUNNING", "Running")
        if status_info["monitoring_active"]
        else texts.get_text("ADMIN_MAINTENANCE_MONITORING_STOPPED", "Stopped")
    )
    ...
    message_text = texts.get_text(
        ...
        autostart_status=texts.get_text("ADMIN_MAINTENANCE_STATUS_ENABLED", "Enabled")
        ...
        auto_enable_status=texts.get_text("ADMIN_MAINTENANCE_STATUS_ENABLED", "Enabled")
...
async def toggle_maintenance_mode(callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    if is_active:
        ...
        if success:
            await callback.answer(
                texts.get_text("ADMIN_MAINTENANCE_DISABLED", "Maintenance mode disabled"), show_alert=True
            )
        else:
            await callback.answer(
                texts.get_text("ADMIN_MAINTENANCE_DISABLE_ERROR", "Error disabling maintenance mode"), show_alert=True
            )
    else:
                ...
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text=texts.get_text("ADMIN_CANCEL", "❌ Cancel"), callback_data="maintenance_panel"
                        )
                    ]
                ]
            ),
    ...
    await message.answer(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.get_text("ADMIN_MAINTENANCE_PANEL_BUTTON", "🔧 Maintenance panel"),
...
async def toggle_monitoring(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if status_info["monitoring_active"]:
        ...
        message = (
            texts.get_text("ADMIN_MAINTENANCE_MONITORING_STOPPED", "Monitoring stopped")
            if success
            else texts.get_text("ADMIN_MAINTENANCE_MONITORING_STOP_ERROR", "Error stopping monitoring")
        )
    else:
        success = await maintenance_service.start_monitoring()
        message = (
            texts.get_text("ADMIN_MAINTENANCE_MONITORING_STARTED", "Monitoring started")
            if success
            else texts.get_text("ADMIN_MAINTENANCE_MONITORING_START_ERROR", "Error starting monitoring")
        )
...
async def check_panel_status(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    texts = get_texts(db_user.language)
    await callback.answer(
        texts.get_text("ADMIN_MAINTENANCE_CHECKING_PANEL", "Checking panel status..."), show_alert=False
    )
    ...
    try:
        ...
        message_parts = [
            ...
            status_text,
            texts.get_text("ADMIN_MAINTENANCE_PANEL_RESPONSE_TIME", "⚡ Response time: {time}s").format(
                time=status_data.get("response_time", 0)
            ),
            texts.get_text("ADMIN_MAINTENANCE_PANEL_USERS_ONLINE", "👥 Users online: {count}").format(
        ...
        if attempts_used:
            message_parts.append(
                texts.get_text("ADMIN_MAINTENANCE_PANEL_ATTEMPTS", "🔁 Check attempts: {count}").format(
                    count=attempts_used
                )
            )
        ...
        if status_data.get("api_error"):
            message_parts.append(
                texts.get_text("ADMIN_MAINTENANCE_PANEL_ERROR", "❌ Error: {error}").format(
        ...
        await callback.message.answer(
            texts.get_text("ADMIN_MAINTENANCE_PANEL_CHECK_ERROR", "❌ Status check error: {error}").format(error=str(e))
        )
...
async def send_manual_notification(callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.get_text("ADMIN_MAINTENANCE_NOTIFY_ONLINE", "🟢 Online"),
                    callback_data="manual_notify_online",
                ),
                types.InlineKeyboardButton(
                    text=texts.get_text("ADMIN_MAINTENANCE_NOTIFY_OFFLINE", "🔴 Offline"),
                    callback_data="manual_notify_offline",
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text=texts.get_text("ADMIN_MAINTENANCE_NOTIFY_DEGRADED", "🟡 Issues"),
                    callback_data="manual_notify_degraded",
                ),
                types.InlineKeyboardButton(
                    text=texts.get_text("ADMIN_MAINTENANCE_NOTIFY_MAINTENANCE", "🔧 Maintenance"),
    ...
    await callback.message.edit_text(
        texts.get_text(
            "ADMIN_MAINTENANCE_MANUAL_NOTIFY_TITLE", "📢 <b>Manual notification</b>\n\nSelect status for notification:"
        ),
...
async def handle_manual_notification(callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    await callback.message.edit_text(
        texts.get_text(
            "ADMIN_MAINTENANCE_SENDING_NOTIFY",
            "📢 <b>Sending notification: {status}</b>\n\nEnter notification message or send /skip to send without additional text:",
...
async def process_notification_message(message: types.Message, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    try:
        ...
        if success:
            ...
            await message.answer(
                texts.get_text("ADMIN_MAINTENANCE_NOTIFICATION_ERROR", "❌ Error sending notification")
            )

    except Exception as e:
        logger.error(f"Error sending manual notification: {e}")
        await message.answer(
            texts.get_text("ADMIN_MAINTENANCE_NOTIFICATION_ERROR_DETAIL", "❌ Error: {error}").format(error=str(e))
        )

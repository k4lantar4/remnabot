...
async def show_users_statistics(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    text += (
        texts.t("ADMIN_STATS_USERS_ACTIVE", "- Active: {count} ({rate})").format(
            count=stats["active_users"], rate=active_rate
        )
        + "\n"
    )
    ...
    text += (
        texts.t("ADMIN_STATS_USERS_GROWTH", "- Monthly growth: +{count} ({rate})").format(
            count=stats["new_month"],
            rate=format_percentage(stats["new_month"] / total_users * 100 if total_users > 0 else 0),
        )
        + "\n\n"
    )
        ...
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_STATS_BTN_REFRESH", "🔄 Refresh"), callback_data="admin_stats_users"
                )
            ],
...
async def show_subscriptions_statistics(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    text += (
        texts.t("ADMIN_STATS_SUBS_TOTAL", "- Total subscriptions: {count}").format(count=stats["total_subscriptions"])
        + "\n"
    )
    ...
    text += (
        texts.t("ADMIN_STATS_SUBS_ACTIVE_PAID", "- Active paid: {count}").format(count=stats["paid_subscriptions"])
        + "\n\n"
    )
    ...
    text += (
        texts.t("ADMIN_STATS_SUBS_SALES_MONTH", "- This month: {count}").format(count=stats["purchased_month"]) + "\n\n"
    )
    text += texts.t("ADMIN_STATS_UPDATED_AT", "<b>Updated:</b> {time}").format(time=current_time)
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_STATS_BTN_REFRESH", "🔄 Refresh"), callback_data="admin_stats_subs"
                )
            ],
            [types.InlineKeyboardButton(text=texts.BACK, callback_data="admin_statistics")],
        ]
...
async def show_revenue_statistics(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    text += (
        texts.t("ADMIN_STATS_REVENUE_INCOME", "- Revenue: {amount}").format(
            amount=settings.format_price(month_stats["totals"]["income_toman"])
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_REVENUE_EXPENSES", "- Expenses: {amount}").format(
            amount=settings.format_price(month_stats["totals"]["expenses_toman"])
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_REVENUE_PROFIT", "- Profit: {amount}").format(
            amount=settings.format_price(month_stats["totals"]["profit_toman"])
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_REVENUE_SUBS_INCOME", "- From subscriptions: {amount}").format(
            amount=settings.format_price(month_stats["totals"]["subscription_income_toman"])
        )
        + "\n\n"
    )
    text += texts.t("ADMIN_STATS_REVENUE_TODAY", "<b>Today:</b>") + "\n"
    text += (
        texts.t("ADMIN_STATS_REVENUE_TODAY_TXS", "- Transactions: {count}").format(
            count=month_stats["today"]["transactions_count"]
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_REVENUE_TODAY_INCOME", "- Revenue: {amount}").format(
            amount=settings.format_price(month_stats["today"]["income_toman"])
        )
        + "\n\n"
    )
    text += texts.t("ADMIN_STATS_REVENUE_ALL_TIME", "<b>All time:</b>") + "\n"
    text += (
        texts.t("ADMIN_STATS_REVENUE_TOTAL_INCOME", "- Total revenue: {amount}").format(
            amount=settings.format_price(all_time_stats["totals"]["income_toman"])
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_REVENUE_TOTAL_PROFIT", "- Total profit: {amount}").format(
            amount=settings.format_price(all_time_stats["totals"]["profit_toman"])
        )
        + "\n\n"
    )
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_STATS_BTN_REFRESH", "🔄 Refresh"), callback_data="admin_stats_revenue"
                )
            ],
            [types.InlineKeyboardButton(text=texts.BACK, callback_data="admin_statistics")],
        ]
...
async def show_referral_statistics(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    text += (
        texts.t("ADMIN_STATS_REF_USERS_WITH_REFS", "- Users with referrals: {count}").format(
            count=stats["users_with_referrals"]
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_REF_ACTIVE_REFERRERS", "- Active referrers: {count}").format(
            count=stats["active_referrers"]
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_REF_TOTAL_PAID", "- Total paid: {amount}").format(
            amount=settings.format_price(stats["total_paid_toman"])
        )
        + "\n\n"
    )
    text += texts.t("ADMIN_STATS_REF_PERIOD", "<b>Period earnings:</b>") + "\n"
    text += (
        texts.t("ADMIN_STATS_REF_TODAY", "- Today: {amount}").format(
            amount=settings.format_price(stats["today_earnings_toman"])
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_REF_WEEK", "- This week: {amount}").format(
            amount=settings.format_price(stats["week_earnings_toman"])
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_REF_MONTH", "- This month: {amount}").format(
            amount=settings.format_price(stats["month_earnings_toman"])
        )
        + "\n\n"
    )
    text += texts.t("ADMIN_STATS_REF_AVERAGES", "<b>Averages:</b>") + "\n"
    text += (
        texts.t("ADMIN_STATS_REF_PER_REFERRER", "- Per referrer: {amount}").format(
            amount=settings.format_price(int(avg_per_referrer))
        )
        + "\n\n"
    )
    ...
    if stats["top_referrers"]:
        for i, referrer in enumerate(stats["top_referrers"][:5], 1):
            ...
            text += (
                texts.t("ADMIN_STATS_REF_TOP_ITEM", "{i}. {name}: {amount} ({count} ref.)").format(
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_STATS_BTN_REFRESH", "🔄 Refresh"), callback_data="admin_stats_referrals"
                )
            ],
            [types.InlineKeyboardButton(text=texts.BACK, callback_data="admin_statistics")],
        ]
...
async def show_summary_statistics(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    text += (
        texts.t("ADMIN_STATS_SUMMARY_USERS_TOTAL", "- Total: {count}").format(count=user_stats["total_users"]) + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_SUMMARY_USERS_ACTIVE", "- Active: {count}").format(count=user_stats["active_users"]) + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_SUMMARY_USERS_NEW_MONTH", "- New this month: {count}").format(
            count=user_stats["new_month"]
        )
        + "\n\n"
    )
    text += texts.t("ADMIN_STATS_SUMMARY_SUBS", "<b>Subscriptions:</b>") + "\n"
    text += (
        texts.t("ADMIN_STATS_SUMMARY_SUBS_ACTIVE", "- Active: {count}").format(count=sub_stats["active_subscriptions"])
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_SUMMARY_SUBS_PAID", "- Paid: {count}").format(count=sub_stats["paid_subscriptions"]) + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_SUMMARY_SUBS_CONVERSION", "- Conversion: {rate}").format(
            rate=format_percentage(conversion_rate)
        )
        + "\n\n"
    )
    text += texts.t("ADMIN_STATS_SUMMARY_FINANCE", "<b>Finances (month):</b>") + "\n"
    text += (
        texts.t("ADMIN_STATS_SUMMARY_FINANCE_INCOME", "- Revenue: {amount}").format(
            amount=settings.format_price(revenue_stats["totals"]["income_toman"])
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_SUMMARY_FINANCE_ARPU", "- ARPU: {amount}").format(amount=settings.format_price(int(arpu)))
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_SUMMARY_FINANCE_TXS", "- Transactions: {count}").format(
            count=sum(data["count"] for data in revenue_stats["by_type"].values())
        )
        + "\n\n"
    )
    text += texts.t("ADMIN_STATS_SUMMARY_GROWTH", "<b>Growth:</b>") + "\n"
    text += (
        texts.t("ADMIN_STATS_SUMMARY_GROWTH_USERS", "- Users: +{count} this month").format(
            count=user_stats["new_month"]
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_STATS_SUMMARY_GROWTH_SALES", "- Sales: +{count} this month").format(
            count=sub_stats["purchased_month"]
        )
        + "\n\n"
    )
    text += texts.t("ADMIN_STATS_UPDATED_AT", "<b>Updated:</b> {time}").format(time=current_time)
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_STATS_BTN_REFRESH", "🔄 Refresh"), callback_data="admin_stats_summary"
                )
            ],
            [types.InlineKeyboardButton(text=texts.BACK, callback_data="admin_statistics")],
        ]
...
async def show_revenue_by_period(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    text += (
        texts.t("ADMIN_STATS_PERIOD_TOTAL", "- Total revenue: {amount}").format(
            amount=settings.format_price(total_revenue)
        )
        + "\n"
    )
    text += texts.t("ADMIN_STATS_PERIOD_DAYS", "- Days with data: {count}").format(count=len(revenue_data)) + "\n"
    text += (
        texts.t("ADMIN_STATS_PERIOD_DAILY_AVG", "- Average daily revenue: {amount}").format(
            amount=settings.format_price(int(avg_daily))
        )
        + "\n\n"
    )
    ...
    if len(revenue_data) > 10:
        text += texts.t("ADMIN_STATS_PERIOD_MORE_DAYS", "... and {count} more days").format(
    ...
    await callback.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.t("ADMIN_STATS_BTN_OTHER_PERIOD", "📊 Other period"),
                        callback_data="admin_revenue_period",
                    )
                ],
                [
                    types.InlineKeyboardButton(
                        text=texts.t("ADMIN_STATS_BTN_TO_REVENUE", "⬅️ To revenue"), callback_data="admin_stats_revenue"
                    )
                ],
            ]
        ),

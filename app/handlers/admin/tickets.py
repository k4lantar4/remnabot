...
async def view_admin_ticket(
    ...
    ticket_text += (
        texts.t("TICKET_CARD_TELEGRAM_ID", "🆔 Telegram ID: {id}").format(id=f"<code>{telegram_id_display}</code>")
        + "\n"
    )
    if username_value:
    ...
    ticket_text += (
        texts.t("TICKET_CARD_STATUS", "📊 Status: {emoji} {status}").format(
    ...
    ticket_text += (
        texts.t("TICKET_CARD_UPDATED", "🔄 Updated: {date}").format(date=ticket.updated_at.strftime("%d.%m.%Y %H:%M"))
        + "\n\n"
    )
    ...
    if ticket.is_user_reply_blocked:
        if ticket.user_reply_block_permanent:
            ticket_text += (
                texts.t("TICKET_USER_BLOCKED_PERM", "🚫 User is permanently blocked from replying to this ticket")
                + "\n"
            )
        elif ticket.user_reply_block_until:
            ticket_text += (
                texts.t("TICKET_USER_BLOCKED_UNTIL", "⏳ Blocked until: {date}").format(
                    date=ticket.user_reply_block_until.strftime("%d.%m.%Y %H:%M")
                )
                + "\n"
            )
    ...
    if ticket.messages:
        ticket_text += (
            texts.t("TICKET_MESSAGES_COUNT", "💬 Messages ({count}):").format(count=len(ticket.messages)) + "\n\n"
        )

        for msg in ticket.messages:
            sender = (
                texts.t("TICKET_SENDER_USER", "👤 User")
                if msg.is_user_message
                else texts.t("TICKET_SENDER_SUPPORT", "🛠️ Support")
            )
            ...
            keyboard.inline_keyboard.insert(
                ...
                [
                    types.InlineKeyboardButton(
                        text=texts.t("TICKET_ATTACHMENTS", "📎 Attachments"),
...
async def handle_admin_ticket_reply(message: types.Message, state: FSMContext, db_user: User, db: AsyncSession):
    ...
    if len(reply_text) < 1 and not media_file_id:
        texts = get_texts(db_user.language)
        await message.answer(texts.t("TICKET_REPLY_TOO_SHORT", "Reply must contain at least 5 characters. Try again:"))
        return
        ...
        if not data.get("reply_mode"):
            ...
            if ok:
                await message.answer(
                    texts.t("USER_BLOCKED_FOR_MINUTES", "✅ User blocked for {minutes} minutes").format(minutes=minutes)
                )
            else:
        ...
        await message.answer(
            ...
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text=texts.t("VIEW_TICKET", "👁️ View ticket"), callback_data=f"admin_view_ticket_{ticket_id}"
                        )
                    ],
                    [
                        types.InlineKeyboardButton(
                            text=texts.t("BACK_TO_TICKETS", "⬅️ Back to tickets"), callback_data="admin_tickets"
                        )
                    ],
                ]
            ),
...
async def mark_ticket_as_answered(callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    try:
        ...
        if success:
            texts = get_texts(db_user.language)
            await callback.answer(texts.t("TICKET_MARKED_ANSWERED", "✅ Ticket marked as answered."), show_alert=True)

            # Update message
            await view_admin_ticket(callback, db_user, db, state)
        else:
            texts = get_texts(db_user.language)
            await callback.answer(texts.t("TICKET_UPDATE_ERROR", "❌ Error updating ticket."), show_alert=True)
...
async def close_all_open_admin_tickets(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if closed_count == 0:
        await callback.answer(
            texts.t("ADMIN_CLOSE_ALL_OPEN_TICKETS_EMPTY", "ℹ️ No open tickets to close."), show_alert=True
        )
        return
    ...
    success_text = texts.t("ADMIN_CLOSE_ALL_OPEN_TICKETS_SUCCESS", "✅ Closed open tickets: {count}").format(
        count=closed_count
    )
    ...
    notification_keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.t("BTN_DELETE", "🗑 Delete"), callback_data="admin_support_delete_msg"
                )
            ]
        ]
    )
...
async def close_admin_ticket(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    try:
        ...
        if success:
            ...
            texts = get_texts(db_user.language)
            await callback.answer(texts.t("TICKET_CLOSE_ERROR", "❌ Error closing ticket."), show_alert=True)
    ...
    await callback.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.t("CANCEL_REPLY", "❌ Cancel input"), callback_data="cancel_admin_ticket_reply"
                    )
                ]
            ]
        ),
...
async def handle_admin_block_duration_input(message: types.Message, state: FSMContext, db_user: User, db: AsyncSession):
    ...
    try:
        ...
        try:
            ...
            ticket_text += (
                texts.t("TICKET_CARD_STATUS", "📊 Status: {emoji} {status}").format(
                    emoji=updated.status_emoji, status=status_text
                )
                + "\n"
            )
            ticket_text += (
                texts.t("TICKET_CARD_CREATED", "📅 Created: {date}").format(
                    date=updated.created_at.strftime("%d.%m.%Y %H:%M")
                )
                + "\n"
            )
            ticket_text += (
                texts.t("TICKET_CARD_UPDATED", "🔄 Updated: {date}").format(
                    date=updated.updated_at.strftime("%d.%m.%Y %H:%M")
                )
                + "\n"
            )
            if updated.user and updated.user.telegram_id:
                ticket_text += (
                    texts.t("TICKET_CARD_TELEGRAM_ID", "🆔 Telegram ID: {id}").format(
                        id=f"<code>{updated.user.telegram_id}</code>"
                    )
                    + "\n"
                )
                if updated.user.username:
                    ...
                    ticket_text += (
                        texts.t("TICKET_CARD_USERNAME", "📱 Username: @{username}").format(username=safe_username)
                        + "\n"
                    )
                    pm_link = f'<a href="tg://resolve?domain={safe_username}">tg://resolve?domain={safe_username}</a>'
                    ticket_text += texts.t("TICKET_CARD_PM_LINK", "🔗 PM: {link}").format(link=pm_link) + "\n"
                    ...
                    chat_link_html = f'<a href="{chat_link}">{chat_link}</a>'
                    ticket_text += (
                        texts.t("TICKET_CARD_CHAT_BY_ID", "🔗 Chat by ID: {link}").format(link=chat_link_html) + "\n"
                    )
            ticket_text += "\n"
            if updated.is_user_reply_blocked:
                if updated.user_reply_block_permanent:
                    ticket_text += (
                        texts.t(
                            "TICKET_USER_BLOCKED_PERM", "🚫 User is permanently blocked from replying to this ticket"
                        )
                        + "\n"
                    )
                    ...
                    ticket_text += (
                        texts.t("TICKET_USER_BLOCKED_UNTIL", "⏳ Blocked until: {date}").format(
                            date=updated.user_reply_block_until.strftime("%d.%m.%Y %H:%M")
                        )
                        + "\n"
                    )
            if updated.messages:
                ticket_text += (
                    texts.t("TICKET_MESSAGES_COUNT", "💬 Messages ({count}):").format(count=len(updated.messages))
                    + "\n\n"
                )
            ...
            try:
                if updated.user and updated.user.telegram_id and updated.user.username:
                    ...
                    pm_url = f"tg://resolve?domain={safe_username}"
                    buttons_row.append(
                        types.InlineKeyboardButton(text=texts.t("BTN_WRITE_PM", "✉ Write PM"), url=pm_url)
                    )
                    profile_url = f"tg://user?id={updated.user.telegram_id}"
                    buttons_row.append(
                        types.InlineKeyboardButton(text=texts.t("BTN_PROFILE", "👤 Profile"), url=profile_url)
                    )
                    if buttons_row:
            ...
            if has_photos:
                try:
                    ...
                    pass
            blocked_msg = texts.t("USER_BLOCKED_FOR_MINUTES", "✅ User blocked for {minutes} minutes").format(
                minutes=minutes
            )
    ...
    try:
        ...
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.t("VIEW_TICKET", "👁️ View ticket"), callback_data=f"view_ticket_{ticket.id}"
                    )
                ],
                [
                    types.InlineKeyboardButton(
                        text=texts.t("CLOSE_NOTIFICATION", "❌ Close notification"),
...
def register_handlers(dp: Dispatcher):
            ...
            try:
                kb = types.InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            types.InlineKeyboardButton(
                                text=texts.t("DELETE_MESSAGE", "🗑 Delete"),
                ...
                await callback.message.bot.send_message(
                    ...
                    text=texts.t("ATTACHMENTS_SENT", "Attachments sent."),
                    reply_markup=kb,
                )
            except Exception:
    ...
    async def admin_delete_message(callback: types.CallbackQuery, db: AsyncSession):

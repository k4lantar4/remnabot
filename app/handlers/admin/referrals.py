...
async def show_referral_statistics(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    try:
        ...
        text += (
            texts.t("ADMIN_REFERRAL_USERS_WITH_REFERRALS", "- Users with referrals: {count}").format(
                count=stats.get("users_with_referrals", 0)
            )
            + "\n"
        )
        text += (
            texts.t("ADMIN_REFERRAL_ACTIVE_REFERRERS", "- Active referrers: {count}").format(
                count=stats.get("active_referrers", 0)
            )
            + "\n"
        )
        text += (
            texts.t("ADMIN_REFERRAL_TOTAL_PAID", "- Total paid: {amount}").format(
                amount=settings.format_price(stats.get("total_paid_toman", 0))
            )
            + "\n\n"
        )
        text += texts.t("ADMIN_REFERRAL_PERIOD_TITLE", "<b>Period earnings:</b>") + "\n"
        text += (
            texts.t("ADMIN_REFERRAL_TODAY", "- Today: {amount}").format(
                amount=settings.format_price(stats.get("today_earnings_toman", 0))
            )
            + "\n"
        )
        text += (
            texts.t("ADMIN_REFERRAL_WEEK", "- This week: {amount}").format(
                amount=settings.format_price(stats.get("week_earnings_toman", 0))
            )
            + "\n"
        )
        text += (
            texts.t("ADMIN_REFERRAL_MONTH", "- This month: {amount}").format(
                amount=settings.format_price(stats.get("month_earnings_toman", 0))
            )
            + "\n\n"
        )
        text += texts.t("ADMIN_REFERRAL_AVERAGES", "<b>Averages:</b>") + "\n"
        text += (
            texts.t("ADMIN_REFERRAL_PER_REFERRER", "- Per referrer: {amount}").format(
        ...
        if top_referrers:
            for i, referrer in enumerate(top_referrers[:5], 1):
                ...
                if count > 0:
                    text += (
                        texts.t("ADMIN_REFERRAL_TOP_ITEM", "{i}. ID {user_id}: {amount} ({count} ref.)").format(
        ...
        text += (
            texts.t("ADMIN_REFERRAL_MIN_TOPUP", "- Minimum top-up: {amount}").format(
                amount=settings.format_price(settings.REFERRAL_MINIMUM_TOPUP_TOMAN)
            )
            + "\n"
        )
        text += (
            texts.t("ADMIN_REFERRAL_FIRST_TOPUP_BONUS", "- First top-up bonus: {amount}").format(
                amount=settings.format_price(settings.REFERRAL_FIRST_TOPUP_BONUS_TOMAN)
            )
            + "\n"
        )
        text += (
            texts.t("ADMIN_REFERRAL_INVITER_BONUS", "- Inviter bonus: {amount}").format(
                amount=settings.format_price(settings.REFERRAL_INVITER_BONUS_TOMAN)
            )
            + "\n"
        )
        text += (
            texts.t("ADMIN_REFERRAL_COMMISSION", "- Purchase commission: {percent}%").format(
                percent=settings.REFERRAL_COMMISSION_PERCENT
            )
            + "\n"
        )
        notifications_status = (
            texts.t("ADMIN_REFERRAL_NOTIFICATIONS_ENABLED", "✅ Enabled")
            if settings.REFERRAL_NOTIFICATIONS_ENABLED
            else texts.t("ADMIN_REFERRAL_NOTIFICATIONS_DISABLED", "❌ Disabled")
        )
        text += (
            texts.t("ADMIN_REFERRAL_NOTIFICATIONS", "- Notifications: {status}").format(status=notifications_status)
            + "\n\n"
        )
            ...
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.t("ADMIN_REFERRAL_BTN_REFRESH", "🔄 Refresh"), callback_data="admin_referrals"
                    )
                ],
                [
                    types.InlineKeyboardButton(
                        text=texts.t("ADMIN_REFERRAL_BTN_TOP", "👥 Top referrers"), callback_data="admin_referrals_top"
                    )
                ],
                [
                    types.InlineKeyboardButton(
                        text=texts.t("ADMIN_REFERRAL_BTN_SETTINGS", "⚙️ Settings"),
        ...
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.t("ADMIN_REFERRAL_BTN_RETRY", "🔄 Retry"), callback_data="admin_referrals"
                    )
                ],
                [types.InlineKeyboardButton(text=texts.BACK, callback_data="admin_panel")],
            ]
    ...
    try:
        ...
        if top_referrers:
            for i, referrer in enumerate(top_referrers[:20], 1):
                ...
                text += (
                    texts.t(
                        "ADMIN_REFERRAL_TOP_ITEM_FULL", "{emoji}{i}. {display}\n   💰 {amount} | 👥 {count} ref."
                    ).format(emoji=emoji, i=i, display=display_text, amount=settings.format_price(earned), count=count)
                    + "\n\n"
                )
        else:
        ...
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.t("ADMIN_REFERRAL_BTN_TO_STATS", "⬅️ To statistics"), callback_data="admin_referrals"
                    )
                ]
            ]
        )
...
async def show_referral_settings(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    text += (
        texts.t("ADMIN_REFERRAL_BONUSES_MIN_TOPUP", "• Minimum top-up for participation: {amount}").format(
            amount=settings.format_price(settings.REFERRAL_MINIMUM_TOPUP_TOMAN)
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_REFERRAL_BONUSES_FIRST_TOPUP", "• Referral first top-up bonus: {amount}").format(
            amount=settings.format_price(settings.REFERRAL_FIRST_TOPUP_BONUS_TOMAN)
        )
        + "\n"
    )
    text += (
        texts.t("ADMIN_REFERRAL_BONUSES_INVITER", "• Inviter bonus for first top-up: {amount}").format(
            amount=settings.format_price(settings.REFERRAL_INVITER_BONUS_TOMAN)
        )
        + "\n\n"
    )
    ...
    text += (
        texts.t("ADMIN_REFERRAL_COMMISSIONS_PERCENT", "• Percent from each referral purchase: {percent}%").format(
            percent=settings.REFERRAL_COMMISSION_PERCENT
        )
        + "\n\n"
    )
    ...
    text += (
        texts.t("ADMIN_REFERRAL_NOTIFICATIONS_STATUS", "• Status: {status}").format(status=notifications_status) + "\n"
    )
    text += (
        texts.t("ADMIN_REFERRAL_NOTIFICATIONS_ATTEMPTS", "• Send attempts: {attempts}").format(
            attempts=getattr(settings, "REFERRAL_NOTIFICATION_RETRY_ATTEMPTS", 3)
        )
        + "\n\n"
    )
    text += texts.t(
        "ADMIN_REFERRAL_SETTINGS_HINT", "<i>💡 To change settings, edit the .env file and restart the bot</i>"
    )

            ...
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=texts.t("ADMIN_SUPPORT_TICKETS", "🎫 Support Tickets"), callback_data="admin_tickets"
                    )
                ],
...
async def rules_stats_command(message: types.Message, db_user: User, db: AsyncSession):
    try:
        ...
        if "error" in stats:
            texts = get_texts(db_user.language)
            await message.reply(
                texts.t("ADMIN_RULES_STATS_ERROR", "❌ Error getting statistics: {error}").format(error=stats["error"])
            )
            return
        ...
        if stats["languages"]:
            ...
            for lang, lang_stats in stats["languages"].items():
                text += texts.t(
                    "ADMIN_RULES_STATS_LANG_LINE", "• <code>{lang}</code>: {count} rules, {chars} characters\n"
                ).format(lang=lang, count=lang_stats["active_count"], chars=lang_stats["content_length"])
...
async def admin_commands_help(message: types.Message, db_user: User, db: AsyncSession):
    ...
    help_text += (
        texts.t(
            "ADMIN_HELP_PANEL_DESCRIPTION",
            "Use the 'Admin Panel' button in the main menu for full access to all functions.",
        )
        + "\n\n"
    )

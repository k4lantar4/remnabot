...
Select action:""",
    ).format(total=len(bots), active=active_count, inactive=len(bots) - active_count)
...
""",
    ...
    if page > 0:
        nav_buttons.append(
            types.InlineKeyboardButton(text="⬅️ Previous", callback_data=f"admin_tenant_bots_list:{page - 1}")
        )
    if end_idx < len(bots):
        nav_buttons.append(
            types.InlineKeyboardButton(text="Next ➡️", callback_data=f"admin_tenant_bots_list:{page + 1}")
        )
    if nav_buttons:
        keyboard_buttons.append(nav_buttons)

    # Back button
    keyboard_buttons.append([types.InlineKeyboardButton(text=texts.BACK, callback_data="admin_tenant_bots_menu")])
...
• Traffic Sold: {traffic_sold} GB""",
    ...
    keyboard_buttons.extend(
        [
            ...
            [types.InlineKeyboardButton(text=texts.BACK, callback_data="admin_tenant_bots_list:0")],
        ]
    )
...
async def activate_tenant_bot(
    ...
    if bot.is_active:
        await callback.answer(texts.t("ADMIN_TENANT_BOT_ALREADY_ACTIVE", "Bot is already active"), show_alert=True)
        return
    ...
    if success:
        ...
        await callback.answer(texts.t("ADMIN_TENANT_BOT_ACTIVATE_ERROR", "❌ Failed to activate bot"), show_alert=True)
...
async def deactivate_tenant_bot(
    ...
    if bot.is_master:
        await callback.answer(
            texts.t("ADMIN_TENANT_BOT_CANNOT_DEACTIVATE_MASTER", "❌ Cannot deactivate master bot"), show_alert=True
        )
        return

    if not bot.is_active:
        await callback.answer(texts.t("ADMIN_TENANT_BOT_ALREADY_INACTIVE", "Bot is already inactive"), show_alert=True)
        return
    ...
    if success:
        ...
        await callback.answer(
            texts.t("ADMIN_TENANT_BOT_DEACTIVATE_ERROR", "❌ Failed to deactivate bot"), show_alert=True
        )
...
Page {page} of {total_pages}
""",
    ).format(bot_name=bot.name, page=page + 1, total_pages=max(1, total_pages))
    ...
    if page > 0:
        nav_buttons.append(
            types.InlineKeyboardButton(text="⬅️ Previous", callback_data=f"admin_tenant_bot_cards:{bot_id}:{page - 1}")
        )
    if end_idx < len(cards):
        nav_buttons.append(
            types.InlineKeyboardButton(text="Next ➡️", callback_data=f"admin_tenant_bot_cards:{bot_id}:{page + 1}")
        )
    if nav_buttons:
        keyboard_buttons.append(nav_buttons)
...
async def toggle_card_to_card(
    ...
    if updated_bot:
        ...
        await callback.answer(texts.t("ADMIN_TENANT_BOT_UPDATE_ERROR", "❌ Failed to update"), show_alert=True)
...
def register_handlers(dp: Dispatcher) -> None:
    ...
    dp.callback_query.register(list_tenant_bots, F.data.startswith("admin_tenant_bots_list"))
    ...
    dp.message.register(process_bot_name, StateFilter(AdminStates.waiting_for_bot_name))

    dp.message.register(process_bot_token, StateFilter(AdminStates.waiting_for_bot_token))

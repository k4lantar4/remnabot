...
async def start_delete_bot(
    ...
    if bot.is_master:
        await callback.answer(
            texts.t("ADMIN_TENANT_BOT_CANNOT_DELETE_MASTER", "❌ Cannot delete master bot"), show_alert=True
        )
        return

    # Get statistics for warning
    user_count_result = await db.execute(select(func.count(User.id)).where(User.bot_id == bot_id))
    user_count = user_count_result.scalar() or 0

    sub_count_result = await db.execute(select(func.count(Subscription.id)).where(Subscription.bot_id == bot_id))
    sub_count = sub_count_result.scalar() or 0

    trans_count_result = await db.execute(select(func.count(Transaction.id)).where(Transaction.bot_id == bot_id))
...
• <b>Hard Delete:</b> Permanently removes bot and all data""",
    ).format(name=bot.name, user_count=user_count, sub_count=sub_count, trans_count=trans_count)
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_CANCEL", "❌ Cancel"), callback_data=f"admin_tenant_bot_detail:{bot_id}"
                )
            ],
        ]
    )
...
The bot can be reactivated from the bot detail menu.""",
                ...
                result_text = texts.t("ADMIN_TENANT_BOT_DELETE_ERROR", "❌ Failed to deactivate bot")
        else:
...
This action cannot be undone.""",
                ...
                result_text = texts.t("ADMIN_TENANT_BOT_DELETE_ERROR", "❌ Failed to delete bot")
        ...
        await callback.answer(texts.t("ADMIN_TENANT_BOT_DELETE_ERROR", "❌ Error deleting bot"), show_alert=True)

...
Total: {count} plan(s)""",
        ).format(name=bot.name, plans="\n\n".join(plans_text), count=len(plans))
    ...
    # Add create button
    keyboard_builder.button(
        text=texts.t("ADMIN_CREATE_PLAN", "➕ Create Plan"), callback_data=f"admin_tenant_bot_plans_create:{bot_id}"
    )

    # Add back button
    keyboard_builder.button(text=texts.BACK, callback_data=f"admin_tenant_bot_detail:{bot_id}")
        ...
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_CANCEL", "❌ Cancel"), callback_data=f"admin_tenant_bot_plans:{bot_id}"
                )
            ]
        ]
    )
...
To cancel, send /cancel""",
        ...
        try:
            ...
            await message.answer(
                texts.t(
                    "ADMIN_TENANT_BOT_PLAN_PERIOD_INVALID", "❌ Invalid period. Please enter a positive number of days."
                )
            )
            return
...
To cancel, send /cancel""",
        ...
        try:
            ...
            await message.answer(
                texts.t(
                    "ADMIN_TENANT_BOT_PLAN_PRICE_INVALID", "❌ Invalid price. Please enter a positive number in Toman."
                )
            )
            return
...
To cancel, send /cancel""",
        ...
        try:
            ...
            await message.answer(
                texts.t(
                    "ADMIN_TENANT_BOT_PLAN_DEVICES_INVALID", "❌ Invalid device limit. Please enter a positive number."
                )
            )
            return
...
Click below to return to plans:""",
            ...
            await message.answer(
                texts.t("ADMIN_TENANT_BOT_PLAN_CREATE_ERROR", "❌ Error creating plan. Please try again.")
            )
...
async def show_plan_detail(
    ...
    if not plan or plan.bot_id != bot_id:
        await callback.answer(texts.t("ADMIN_TENANT_BOT_PLAN_NOT_FOUND", "❌ Plan not found"), show_alert=True)
        return
...
Select an action:""",
    ...
    toggle_text = (
        texts.t("ADMIN_DEACTIVATE", "❌ Deactivate") if plan.is_active else texts.t("ADMIN_ACTIVATE", "✅ Activate")
    )
    keyboard_builder.button(text=toggle_text, callback_data=f"admin_tenant_bot_plan_toggle:{bot_id}:{plan_id}")

    # Delete button
    keyboard_builder.button(
        text=texts.t("ADMIN_DELETE", "🗑️ Delete"), callback_data=f"admin_tenant_bot_plan_delete:{bot_id}:{plan_id}"
    )

    # Back button
    keyboard_builder.button(text=texts.BACK, callback_data=f"admin_tenant_bot_plans:{bot_id}")
...
async def toggle_plan_status(
    ...
    await callback.answer(texts.t("ADMIN_TENANT_BOT_PLAN_STATUS_TOGGLED", f"✅ Plan {status_text}"), show_alert=False)
...
Are you sure you want to delete this plan?""",
    ).format(name=plan.name, period=plan.period_days, price=plan.price_toman)
...
Click below to return to plans:""",
        ...
        await callback.message.edit_text(text, reply_markup=keyboard, parse_mode="HTML")
        await callback.answer(texts.t("ADMIN_TENANT_BOT_PLAN_DELETE_SUCCESS", "✅ Plan deleted"), show_alert=False)
    else:
        await callback.answer(texts.t("ADMIN_TENANT_BOT_PLAN_DELETE_ERROR", "❌ Error deleting plan"), show_alert=True)

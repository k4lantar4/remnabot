...
async def show_bot_detail(
    ...
    active_subs_result = await db.execute(
        select(func.count(Subscription.id)).where(
            and_(Subscription.bot_id == bot_id, Subscription.status == SubscriptionStatus.ACTIVE.value)
        )
    )
...
• Traffic Consumed: {traffic_consumed} GB""",
    ...
    # Back button
    keyboard_buttons.append([types.InlineKeyboardButton(text=texts.BACK, callback_data="admin_tenant_bots_list")])

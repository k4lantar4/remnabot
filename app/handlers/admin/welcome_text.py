...
async def show_welcome_text_panel(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    status_text = (
        texts.t("ADMIN_WELCOME_TEXT_ENABLED", "enabled")
        if welcome_settings["is_enabled"]
        else texts.t("ADMIN_WELCOME_TEXT_DISABLED", "disabled")
    )
...
async def toggle_welcome_text(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    action_text = (
        texts.t("ADMIN_WELCOME_TEXT_ENABLED_PLURAL", "enabled")
        if new_status
        else texts.t("ADMIN_WELCOME_TEXT_DISABLED_PLURAL", "disabled")
    )
...
async def process_welcome_text_edit(message: types.Message, state: FSMContext, db_user: User, db: AsyncSession):
    ...
    if not is_valid:
        await message.answer(
            texts.t("ADMIN_WELCOME_TEXT_HTML_ERROR", "❌ HTML markup error:\n\n{error}").format(error=error_msg)
        )
        return
    ...
    if success:
        ...
        await message.answer(
            ...
            ).format(emoji=status_emoji, status=status_text, text=new_text, placeholders=placeholders_text),
            reply_markup=get_welcome_text_keyboard(db_user.language, welcome_settings["is_enabled"]),
            parse_mode="HTML",
        )
    else:

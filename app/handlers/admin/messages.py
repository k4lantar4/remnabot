...
async def show_messages_menu(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    text = (
        texts.t("ADMIN_MESSAGES_MENU_TITLE", "📨 <b>Broadcast management</b>")
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_MENU_DESCRIPTION", "Choose broadcast type:")
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_MENU_ALL_USERS", "- <b>All users</b> - broadcast to all active users")
        + "\n"
        + texts.t("ADMIN_MESSAGES_MENU_BY_SUBSCRIPTIONS", "- <b>By subscriptions</b> - filter by subscription type")
        + "\n"
        + texts.t("ADMIN_MESSAGES_MENU_BY_CRITERIA", "- <b>By criteria</b> - custom filters")
        + "\n"
        + texts.t("ADMIN_MESSAGES_MENU_HISTORY", "- <b>History</b> - view previous broadcasts")
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_MENU_WARNING", "⚠️ Be careful with mass broadcasts!")
    )

    await callback.message.edit_text(
        text, reply_markup=get_admin_messages_keyboard(db_user.language), parse_mode="HTML"
    )
...
async def show_broadcast_targets(callback: types.CallbackQuery, db_user: User, state: FSMContext):
    ...
    await callback.message.edit_text(
        texts.t(
            "ADMIN_MESSAGES_TARGET_SELECTION", "🎯 <b>Select target audience</b>\n\nChoose user category for broadcast:"
        ),
        ...
        keyboard = [
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_MESSAGES_HISTORY_BACK", "⬅️ Back"), callback_data="admin_messages"
                )
            ]
        ]
    else:
        text = texts.t(
            "ADMIN_MESSAGES_HISTORY_PAGE", "📋 <b>Broadcast history</b> (page {page}/{total_pages})\n\n"
...
async def show_custom_broadcast(callback: types.CallbackQuery, db_user: User, state: FSMContext, db: AsyncSession):
    ...
    text = (
        texts.t("ADMIN_MESSAGES_CUSTOM_TITLE", "📝 <b>Broadcast by criteria</b>")
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_FILTERS", "📊 <b>Available filters:</b>")
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_BY_REGISTRATION", "👥 <b>By registration:</b>")
        + "\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_TODAY", "• Today: {count} users").format(count=stats["today"])
        + "\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_WEEK", "• Last week: {count} users").format(count=stats["week"])
        + "\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_MONTH", "• Last month: {count} users").format(count=stats["month"])
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_BY_ACTIVITY", "💼 <b>By activity:</b>")
        + "\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_ACTIVE_TODAY", "• Active today: {count} users").format(
            count=stats["active_today"]
        )
        + "\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_INACTIVE_WEEK", "• Inactive 7+ days: {count} users").format(
            count=stats["inactive_week"]
        )
        + "\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_INACTIVE_MONTH", "• Inactive 30+ days: {count} users").format(
            count=stats["inactive_month"]
        )
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_BY_SOURCE", "🔗 <b>By source:</b>")
        + "\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_REFERRALS", "• Via referrals: {count} users").format(count=stats["referrals"])
        + "\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_DIRECT", "• Direct registration: {count} users").format(count=stats["direct"])
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_CUSTOM_SELECT_CRITERIA", "Choose criteria for filtering:")
    )
...
async def select_custom_criteria(callback: types.CallbackQuery, db_user: User, state: FSMContext, db: AsyncSession):
    ...
    await callback.message.edit_text(
        texts.t("ADMIN_MESSAGES_CREATE_TITLE", "📨 <b>Create broadcast</b>\n\n")
        + texts.t("ADMIN_MESSAGES_CREATE_CRITERIA", "🎯 <b>Criteria:</b> {criteria}\n").format(
            criteria=criteria_names.get(criteria, criteria)
        )
        + texts.t("ADMIN_MESSAGES_CREATE_RECIPIENTS", "👥 <b>Recipients:</b> {count}\n\n").format(count=user_count)
        + texts.t("ADMIN_MESSAGES_CREATE_PROMPT", "Enter message text for broadcast:\n\n")
        + texts.t("ADMIN_MESSAGES_CREATE_HTML_HINT", "<i>HTML markup is supported</i>"),
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.t("ADMIN_MESSAGES_CREATE_CANCEL", "❌ Cancel"), callback_data="admin_messages"
                    )
                ]
            ]
        ),
...
async def select_broadcast_target(callback: types.CallbackQuery, db_user: User, state: FSMContext, db: AsyncSession):
    ...
    await callback.message.edit_text(
        texts.t("ADMIN_MESSAGES_CREATE_TITLE", "📨 <b>Create broadcast</b>\n\n")
        + texts.t("ADMIN_MESSAGES_CREATE_AUDIENCE", "🎯 <b>Audience:</b> {audience}\n").format(
            audience=target_names.get(target, target)
        )
        + texts.t("ADMIN_MESSAGES_CREATE_RECIPIENTS", "👥 <b>Recipients:</b> {count}\n\n").format(count=user_count)
        + texts.t("ADMIN_MESSAGES_CREATE_PROMPT", "Enter message text for broadcast:\n\n")
        + texts.t("ADMIN_MESSAGES_CREATE_HTML_HINT", "<i>HTML markup is supported</i>"),
...
async def process_broadcast_message(message: types.Message, db_user: User, state: FSMContext, db: AsyncSession):
    ...
    await message.answer(
        texts.t("ADMIN_MESSAGES_ADD_MEDIA_TITLE", "🖼️ <b>Adding media file</b>\n\n")
        + texts.t(
            "ADMIN_MESSAGES_ADD_MEDIA_DESCRIPTION",
            "You can add a photo, video or document to the message.\nOr skip this step.\n\n",
        )
        + texts.t("ADMIN_MESSAGES_ADD_MEDIA_SELECT", "Choose media type:"),
...
async def process_broadcast_media(message: types.Message, db_user: User, state: FSMContext):
    ...
    if message.photo and expected_type == "photo":
        ...
        await message.answer(
            texts.t("ADMIN_MESSAGES_MEDIA_WRONG_TYPE", "❌ Please send {type} as instructed.").format(
                type=media_type_names.get(expected_type, expected_type)
            )
        )
        return
...
async def show_media_preview(message: types.Message, db_user: User, state: FSMContext):
    ...
    preview_text = (
        texts.t("ADMIN_MESSAGES_MEDIA_ADDED", "🖼️ <b>Media file added</b>\n\n")
        + texts.t("ADMIN_MESSAGES_MEDIA_TYPE", "📎 <b>Type:</b> {type}\n").format(
            type=media_type_names.get(media_type, media_type)
        )
        + texts.t("ADMIN_MESSAGES_MEDIA_SAVED", "✅ File saved and ready to send\n\n")
        + texts.t("ADMIN_MESSAGES_MEDIA_NEXT", "What to do next?")
    )
...
async def handle_change_media(callback: types.CallbackQuery, db_user: User, state: FSMContext):
    texts = get_texts(db_user.language)
    await callback.message.edit_text(
        texts.t("ADMIN_MESSAGES_CHANGE_MEDIA_TITLE", "🖼️ <b>Changing media file</b>\n\n")
        + texts.t("ADMIN_MESSAGES_CHANGE_MEDIA_SELECT", "Choose new media type:"),
        reply_markup=get_broadcast_media_keyboard(db_user.language),
        parse_mode="HTML",
    )
    await callback.answer()
...
async def show_button_selector_callback(callback: types.CallbackQuery, db_user: User, state: FSMContext):
    ...
    if has_media:
        ...
        media_info = (
            "\n"
            + texts.t("ADMIN_MESSAGES_PREVIEW_MEDIA", "\n🖼️ <b>Media file:</b> {media_type}").format(
    ...
    text = (
        texts.t("ADMIN_MESSAGES_BUTTON_SELECTOR_TITLE", "📘 <b>Select additional buttons</b>")
        + "\n\n"
        + texts.t(
            "ADMIN_MESSAGES_BUTTON_SELECTOR_DESCRIPTION", "Choose buttons that will be added to the broadcast message:"
        )
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_BUTTON_BALANCE_DESC", "💰 <b>Top up balance</b> — opens top-up methods")
        + "\n"
        + texts.t("ADMIN_MESSAGES_BUTTON_REFERRALS_DESC", "🤝 <b>Referrals</b> — opens referral program")
        + "\n"
        + texts.t("ADMIN_MESSAGES_BUTTON_PROMOCODE_DESC", "🎫 <b>Promo code</b> — opens promo code input form")
        + "\n"
        + texts.t("ADMIN_MESSAGES_BUTTON_CONNECT_DESC", "🔗 <b>Connect</b> — helps connect the app")
        + "\n"
        + texts.t("ADMIN_MESSAGES_BUTTON_SUBSCRIPTION_DESC", "📱 <b>Subscription</b> — shows subscription status")
        + "\n"
        + texts.t("ADMIN_MESSAGES_BUTTON_SUPPORT_DESC", "🛠️ <b>Support</b> — connects with support")
        + "\n\n"
        + texts.t(
        ...
        + media_info
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_BUTTON_SELECTOR_PROMPT", 'Choose needed buttons and press "Continue":')
    )
...
async def confirm_button_selection(callback: types.CallbackQuery, db_user: User, state: FSMContext, db: AsyncSession):
    ...
    if has_media:
        ...
        media_info = texts.t("ADMIN_MESSAGES_PREVIEW_MEDIA", "\n🖼️ <b>Media file:</b> {media_type}").format(
            media_type=media_type_names.get(media_type, media_type)
        )
    ...
    if selected_names:
        buttons_info = texts.t("ADMIN_MESSAGES_PREVIEW_BUTTONS", "\n📘 <b>Buttons:</b> {buttons}").format(
            buttons=", ".join(selected_names)
        )
    else:
        buttons_info = texts.t("ADMIN_MESSAGES_PREVIEW_NO_BUTTONS", "\n📘 <b>Buttons:</b> none")

    preview_text = (
        texts.t("ADMIN_MESSAGES_PREVIEW_TITLE", "📨 <b>Broadcast preview</b>")
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_PREVIEW_AUDIENCE", "🎯 <b>Audience:</b> {audience}").format(audience=target_display)
        + "\n"
        + texts.t("ADMIN_MESSAGES_PREVIEW_RECIPIENTS", "👥 <b>Recipients:</b> {count}").format(count=user_count)
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_PREVIEW_MESSAGE", "📝 <b>Message:</b>\n{message}").format(message=message_text)
        + media_info
        + buttons_info
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_PREVIEW_CONFIRM", "Confirm sending?")
    )

    keyboard = [
        [
            types.InlineKeyboardButton(
                text=texts.t("ADMIN_MESSAGES_PREVIEW_SEND", "✅ Send"), callback_data="admin_confirm_broadcast"
            ),
            types.InlineKeyboardButton(
                text=texts.t("ADMIN_MESSAGES_PREVIEW_EDIT_BUTTONS", "📘 Edit buttons"), callback_data="edit_buttons"
            ),
        ]
    ]
    ...
    if has_media:
        keyboard.append(
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_MESSAGES_PREVIEW_CHANGE_MEDIA", "🖼️ Change media"), callback_data="change_media"
                )
            ]
        )
    ...
    keyboard.append(
        [
            types.InlineKeyboardButton(
                text=texts.t("ADMIN_MESSAGES_PREVIEW_CANCEL", "❌ Cancel"), callback_data="admin_messages"
            )
        ]
    )
    ...
    if has_media and media_type == "photo":
        ...
        if media_file_id:
            ...
            await callback.message.edit_text(
                preview_text, reply_markup=types.InlineKeyboardMarkup(inline_keyboard=keyboard), parse_mode="HTML"
            )
    else:
...
async def confirm_broadcast(callback: types.CallbackQuery, db_user: User, state: FSMContext, db: AsyncSession):
    ...
    texts = get_texts(db_user.language)
    await callback.message.edit_text(
        texts.t("ADMIN_MESSAGES_SENDING", "📨 Starting broadcast...\n\n")
        + texts.t("ADMIN_MESSAGES_SENDING_WAIT", "⏳ This may take several minutes."),
        reply_markup=None,
        parse_mode="HTML",
    )
        ...
        async with semaphore:
            for attempt in range(3):
                try:
                    ...
                    logger.warning(f"Telegram rate limit exceeded for {user.telegram_id}, waiting {retry_delay} sec.")
                    ...
                    logger.error(f"Invalid request when broadcasting to user {user.telegram_id}: {e}")
                    return False, user.telegram_id
                except Exception as e:
                    logger.error(f"Error sending broadcast to user {user.telegram_id} (attempt {attempt + 1}/3): {e}")
    ...
    result_text = (
        texts.t("ADMIN_MESSAGES_COMPLETED", "✅ <b>Broadcast completed!</b>")
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_RESULT", "📊 <b>Result:</b>")
        + "\n"
        + texts.t("ADMIN_MESSAGES_RESULT_SENT", "- Sent: {sent}").format(sent=sent_count)
        + "\n"
        + texts.t("ADMIN_MESSAGES_RESULT_FAILED", "- Failed: {failed}").format(failed=failed_count)
        + "\n"
        + texts.t("ADMIN_MESSAGES_RESULT_TOTAL", "- Total users: {total}").format(total=len(users))
        + "\n"
        + texts.t("ADMIN_MESSAGES_RESULT_SUCCESS_RATE", "- Success rate: {rate}%").format(rate=success_rate)
        + media_info
        + "\n\n"
        + texts.t("ADMIN_MESSAGES_RESULT_ADMIN", "<b>Administrator:</b> {admin}").format(admin=db_user.full_name)
    )
    ...
    await callback.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.t("ADMIN_MESSAGES_BACK_TO_BROADCASTS", "📨 Back to broadcasts"),

...
async def show_promocodes_list(callback: types.CallbackQuery, db_user: User, db: AsyncSession, page: int = 1):
    ...
    text = (
        texts.t("ADMIN_PROMOCODES_LIST_HEADER", "🎫 <b>Promocode List</b> (page {page}/{total})").format(
    ...
    for promo in promocodes:
        ...
        type_emoji = {"balance": "💰", "subscription_days": "📅", "trial_subscription": "🎁", "promo_group": "🏷️"}.get(
            promo.type, "🎫"
        )

        text += f"{status_emoji} {type_emoji} <code>{promo.code}</code>\n"
        text += (
            texts.t("ADMIN_PROMOCODES_USES", "📊 Uses: {current}/{max}").format(
        ...
        if promo.type == PromoCodeType.BALANCE.value:
            text += (
                texts.t("ADMIN_PROMOCODES_BONUS", "💰 Bonus: {amount}").format(
                    amount=settings.format_price(promo.balance_bonus_toman)
                )
                + "\n"
            )
        elif promo.type == PromoCodeType.SUBSCRIPTION_DAYS.value:
            text += texts.t("ADMIN_PROMOCODES_DAYS", "📅 Days: {days}").format(days=promo.subscription_days) + "\n"
        elif promo.type == PromoCodeType.PROMO_GROUP.value:
            if promo.promo_group:
                text += (
                    texts.t("ADMIN_PROMOCODES_PROMO_GROUP", "🏷️ Promo group: {name}").format(name=promo.promo_group.name)
                    + "\n"
                )

        if promo.valid_until:
            text += (
                texts.t("ADMIN_PROMOCODES_VALID_UNTIL", "⏰ Until: {date}").format(
        ...
        [
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_BTN_CREATE", "➕ Create"), callback_data="admin_promo_create"
                )
            ],
...
async def show_promocode_management(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    status_text = (
        texts.t("ADMIN_STATUS_ACTIVE", "Active") if promo.is_active else texts.t("ADMIN_STATUS_INACTIVE", "Inactive")
    )
    ...
    if promo.type == PromoCodeType.BALANCE.value:
        text += (
            texts.t("ADMIN_PROMOCODE_BONUS_LABEL", "💰 <b>Bonus:</b> {amount}").format(
                amount=settings.format_price(promo.balance_bonus_toman)
            )
            + "\n"
        )
        ...
        text += (
            texts.t("ADMIN_PROMOCODE_DAYS_LABEL", "📅 <b>Days:</b> {days}").format(days=promo.subscription_days) + "\n"
        )
    elif promo.type == PromoCodeType.PROMO_GROUP.value:
        if promo.promo_group:
            text += (
                texts.t(
                    "ADMIN_PROMOCODE_PROMO_GROUP_LABEL", "🏷️ <b>Promo group:</b> {name} (priority: {priority})"
                ).format(name=promo.promo_group.name, priority=promo.promo_group.priority)
                + "\n"
            )
        elif promo.promo_group_id:
            text += (
                texts.t("ADMIN_PROMOCODE_PROMO_GROUP_ID_LABEL", "🏷️ <b>Promo group ID:</b> {id} (not found)").format(
    ...
    if promo.valid_until:
        text += (
            texts.t("ADMIN_PROMOCODE_VALID_UNTIL_LABEL", "⏰ <b>Valid until:</b> {date}").format(
                date=format_datetime(promo.valid_until)
            )
            + "\n"
        )
    ...
    text += (
        texts.t("ADMIN_PROMOCODE_CREATED_LABEL", "📅 <b>Created:</b> {date}").format(
    ...
    keyboard = [
        [
            types.InlineKeyboardButton(
                text=texts.t("ADMIN_BTN_EDIT", "✏️ Edit"), callback_data=f"promo_edit_{promo.id}"
            ),
            types.InlineKeyboardButton(
                text=texts.t("ADMIN_BTN_TOGGLE_STATUS", "🔄 Toggle status"), callback_data=f"promo_toggle_{promo.id}"
            ),
        ],
        [
            types.InlineKeyboardButton(
                text=texts.t("ADMIN_BTN_STATISTICS", "📊 Statistics"), callback_data=f"promo_stats_{promo.id}"
            ),
            types.InlineKeyboardButton(
                text=texts.t("ADMIN_BTN_DELETE", "🗑️ Delete"), callback_data=f"promo_delete_{promo.id}"
            ),
        ],
...
async def show_promocode_edit_menu(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if promo.type == PromoCodeType.BALANCE.value:
        text += (
            texts.t("ADMIN_PROMOCODE_EDIT_BONUS", "• Bonus: {amount}").format(
                amount=settings.format_price(promo.balance_bonus_toman)
            )
            + "\n"
        )
    elif promo.type in [PromoCodeType.SUBSCRIPTION_DAYS.value, PromoCodeType.TRIAL_SUBSCRIPTION.value]:
        text += texts.t("ADMIN_PROMOCODE_EDIT_DAYS", "• Days: {days}").format(days=promo.subscription_days) + "\n"

    text += (
        texts.t("ADMIN_PROMOCODE_EDIT_USES", "• Uses: {current}/{max}").format(
            current=promo.current_uses, max=promo.max_uses
        )
        + "\n"
    )
    ...
    if promo.valid_until:
        text += (
            texts.t("ADMIN_PROMOCODE_EDIT_UNTIL", "• Until: {date}").format(date=format_datetime(promo.valid_until))
            + "\n"
        )
    else:
    ...
    keyboard = [
        [
            types.InlineKeyboardButton(
                text=texts.t("ADMIN_BTN_EXPIRY_DATE", "📅 Expiry date"), callback_data=f"promo_edit_date_{promo.id}"
            )
        ],
        [
            types.InlineKeyboardButton(
                text=texts.t("ADMIN_BTN_USES_COUNT", "📊 Uses count"), callback_data=f"promo_edit_uses_{promo.id}"
            )
        ],
    ]
    ...
    if promo.type == PromoCodeType.BALANCE.value:
        ...
        keyboard.insert(
            ...
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_BTN_DAYS_COUNT", "📅 Days count"), callback_data=f"promo_edit_days_{promo.id}"
                )
            ],
        )
...
async def start_edit_promocode_date(callback: types.CallbackQuery, db_user: User, state: FSMContext):
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_BTN_CANCEL", "❌ Cancel"), callback_data=f"promo_edit_{promo_id}"
                )
            ]
        ]
    )
...
async def start_promocode_creation(callback: types.CallbackQuery, db_user: User, state: FSMContext):
    texts = get_texts(db_user.language)
    await callback.message.edit_text(
        texts.t("ADMIN_PROMOCODE_CREATE_HEADER", "🎫 <b>Create promocode</b>\n\nSelect promocode type:"),
    ...
    await callback.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.t("ADMIN_BTN_CANCEL", "❌ Cancel"), callback_data="admin_promocodes"
                    )
                ]
            ]
        ),
...
async def process_promocode_code(message: types.Message, db_user: User, state: FSMContext, db: AsyncSession):
    ...
    if not code.isalnum() or len(code) < 3 or len(code) > 20:
        await message.answer(
            texts.t(
                "ADMIN_PROMOCODE_INVALID_CODE", "❌ Code must contain only Latin letters and numbers (3-20 characters)"
            )
        )
        return
    ...
    if promo_type == "balance":
        ...
        for promo_group, user_count in groups_with_counts:
            text += (
                texts.t("ADMIN_PROMOCODE_GROUP_INFO", "• {name} (priority: {priority}, users: {users})").format(
                    name=promo_group.name, priority=promo_group.priority, users=user_count
                )
                + "\n"
            )
            keyboard.append(
    ...
    try:
        ...
        if edit_action == "amount":
            if value < 1 or value > 10000:
                await message.answer(
                    texts.t("ADMIN_PROMOCODE_INVALID_AMOUNT", "❌ Amount must be from 1 to 10,000 rubles")
                )
                return

            await update_promocode(db, promo, balance_bonus_toman=value * 100)
            await message.answer(
                texts.t("ADMIN_PROMOCODE_AMOUNT_CHANGED", "✅ Bonus amount changed to {value} Toman").format(
                    value=value
                ),
                reply_markup=types.InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            types.InlineKeyboardButton(
                                text=texts.t("ADMIN_BTN_TO_PROMOCODE", "🎫 To promocode"),
...
async def process_promocode_expiry(message: types.Message, db_user: User, state: FSMContext, db: AsyncSession):
    ...
    try:
        ...
        if promo_type == "balance":
            summary_text += (
                texts.t("ADMIN_PROMOCODE_SUMMARY_AMOUNT", "💰 <b>Amount:</b> {amount}").format(
            ...
            summary_text += (
                texts.t("ADMIN_PROMOCODE_SUMMARY_DAYS", "📅 <b>Days:</b> {days}").format(
            ...
            summary_text += (
                texts.t("ADMIN_PROMOCODE_SUMMARY_GROUP", "🏷️ <b>Promo group:</b> {name}").format(name=promo_group_name)
                + "\n"
            )

        summary_text += (
            texts.t("ADMIN_PROMOCODE_SUMMARY_USES", "📊 <b>Uses:</b> {uses}").format(uses=promocode.max_uses) + "\n"
        )

        if promocode.valid_until:
            summary_text += (
                texts.t("ADMIN_PROMOCODE_SUMMARY_VALID_UNTIL", "⏰ <b>Valid until:</b> {date}").format(
        ...
        await message.answer(
            ...
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text=texts.t("ADMIN_BTN_TO_PROMOCODES", "🎫 To promocodes"),
...
async def handle_edit_expiry(message: types.Message, db_user: User, state: FSMContext, db: AsyncSession):
    ...
    try:
        ...
        if valid_until:
            expiry_text = texts.t("ADMIN_PROMOCODE_EXPIRY_UNTIL", "until {date}").format(
                date=format_datetime(valid_until)
            )
        else:
            expiry_text = texts.t("ADMIN_PROMOCODE_UNLIMITED", "unlimited")

        await message.answer(
            texts.t("ADMIN_PROMOCODE_EXPIRY_CHANGED", "✅ Promocode validity changed: {expiry}").format(
                expiry=expiry_text
            ),
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
...
async def toggle_promocode_status(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    status_text = (
        texts.t("ADMIN_PROMOCODE_ACTIVATED", "activated")
        if new_status
        else texts.t("ADMIN_PROMOCODE_DEACTIVATED", "deactivated")
    )
    await callback.answer(
        texts.t("ADMIN_PROMOCODE_STATUS_CHANGED", "✅ Promocode {status}").format(status=status_text), show_alert=True
    )
...
async def confirm_delete_promocode(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    text = texts.t(
        ...
        "ID: {promo_id}",
    ).format(code=promo.code, current=promo.current_uses, max=promo.max_uses, status=status_text, promo_id=promo_id)
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                ...
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_BTN_CANCEL", "❌ Cancel"), callback_data=f"promo_manage_{promo.id}"
                ),
            ]
        ]
    )
...
async def delete_promocode_confirmed(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if success:
        await callback.answer(
            texts.t("ADMIN_PROMOCODE_DELETED", "✅ Promocode {code} deleted").format(code=code), show_alert=True
        )
...
async def show_general_promocode_stats(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_BTN_TO_PROMOCODES", "🎫 To promocodes"), callback_data="admin_promo_list"
                )
            ],
            [types.InlineKeyboardButton(text=texts.BACK, callback_data="admin_promocodes")],
        ]

...
def _format_duration(seconds: float, texts) -> str:
    ...
    if minutes:
        if sec:
            return texts.t("ADMIN_RW_DURATION_MIN_SEC", "{minutes} min {seconds} s").format(
...
def _build_auto_sync_view(
    status: RemnaWaveAutoSyncStatus, language: str = "en"
    ...
    if status.last_run_finished_at:
        ...
        result_label = (
            texts.t("ADMIN_RW_RESULT_SUCCESS", "success")
            if status.last_run_success
            else texts.t("ADMIN_RW_RESULT_ERRORS", "with errors")
        )
        error_block = (
        ...
        last_run_text = (
            ...
            else texts.t("ADMIN_RW_LAST_RUN_INFO", "ℹ️ Last run: {time}").format(
    ...
    running_text = (
        texts.t("ADMIN_RW_RUNNING_NOW", "⏳ Running now")
        if status.is_running
        else texts.t("ADMIN_RW_WAITING", "Waiting")
    )
    toggle_text = (
        texts.t("ADMIN_RW_DISABLE", "❌ Disable") if status.enabled else texts.t("ADMIN_RW_ENABLE", "✅ Enable")
    )
    ...
    text = texts.t(
        ...
        status_label=texts.t("ADMIN_RW_STATUS_ENABLED", "Enabled")
        if status.enabled
        else texts.t("ADMIN_RW_STATUS_DISABLED", "Disabled"),
        schedule=times_text,
...
async def confirm_squad_migration(
    ...
    message_lines = [
        ...
        "",
        texts.get_text("ADMIN_SQUAD_MIGRATION_CONFIRM_SOURCE", "• From: {source}").format(source=source_display),
        texts.get_text("ADMIN_SQUAD_MIGRATION_CONFIRM_TARGET", "• To: {target}").format(target=target_display),
        "",
        texts.get_text(
...
async def show_remnawave_menu(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    text = texts.get_text(
        ...
        "Select an action:",
    ).format(status_emoji=status_emoji, message=connection_message, url=api_url_display)
...
async def show_system_stats(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if "error" in stats:
        await callback.message.edit_text(
            texts.t("ADMIN_RW_SYSTEM_STATS_ERROR", "❌ Failed to fetch statistics: {error}").format(
                error=stats["error"]
            ),
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text=texts.t("ADMIN_RW_BACK", "⬅️ Back"), callback_data="admin_remnawave"
                        )
                    ]
                ]
            ),
    ...
    uptime_str = texts.get_text("ADMIN_RW_UPTIME_FORMAT", "{days}d {hours}h").format(
        days=uptime_days, hours=uptime_hours
    )
...
""",
    ...
    keyboard = [
        [
            types.InlineKeyboardButton(
                text=texts.t("ADMIN_RW_BTN_REFRESH", "🔄 Refresh"), callback_data="admin_rw_system"
            )
        ],
        [
...
async def show_traffic_stats(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    try:
        ...
        await callback.message.edit_text(
            texts.t("ADMIN_RW_TRAFFIC_STATS_ERROR", "❌ Failed to fetch traffic statistics: {error}").format(
                error=str(e)
            ),
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
    ...
    if realtime_usage:
        nodes_realtime_text = texts.get_text(
            "ADMIN_RW_TRAFFIC_NODES_REALTIME_HEADER", "\n🌐 <b>Traffic by nodes (realtime):</b>\n"
        )
    ...
    text += texts.get_text("ADMIN_RW_TRAFFIC_UPDATED", "\n🕒 <b>Updated:</b> {updated_at}").format(
    ...
    keyboard = [
        [
            types.InlineKeyboardButton(
                text=texts.get_text("ADMIN_RW_BTN_REFRESH", "🔄 Refresh"), callback_data="admin_rw_traffic"
            )
        ],
        [
            types.InlineKeyboardButton(
                text=texts.get_text("ADMIN_RW_BTN_NODES", "📈 Nodes"), callback_data="admin_rw_nodes"
            ),
            types.InlineKeyboardButton(
                text=texts.get_text("ADMIN_RW_BTN_SYSTEM", "📊 System"), callback_data="admin_rw_system"
            ),
        ],
...
async def show_nodes_management(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if not nodes:
        await callback.message.edit_text(
            texts.t("ADMIN_RW_NODES_NOT_FOUND", "🖥️ Nodes not found or connection error"),
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
    ...
    keyboard.extend(
        [
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_RW_NODES_RESTART_ALL", "🔄 Restart all"),
...
async def show_node_details(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if not node:
        await callback.answer(texts.get_text("ADMIN_RW_NODE_NOT_FOUND", "❌ Node not found"), show_alert=True)
        return
    ...
    online_text = (
        texts.get_text("ADMIN_RW_YES", "Yes") if node["is_node_online"] else texts.get_text("ADMIN_RW_NO", "No")
    )
    xray_text = (
        texts.get_text("ADMIN_RW_XRAY_RUNNING", "Running")
        if node["is_xray_running"]
        else texts.get_text("ADMIN_RW_XRAY_STOPPED", "Stopped")
    )
    connected_text = (
        texts.get_text("ADMIN_RW_YES", "Yes") if node["is_connected"] else texts.get_text("ADMIN_RW_NO", "No")
    )
    connected_emoji = "📡" if node["is_connected"] else "📵"
    disabled_text = (
        texts.get_text("ADMIN_RW_YES", "Yes") if node["is_disabled"] else texts.get_text("ADMIN_RW_NO", "No")
    )
    disabled_emoji = "❌" if node["is_disabled"] else "✅"
    traffic_limit = (
        format_bytes(node["traffic_limit_bytes"])
        if node["traffic_limit_bytes"]
        else texts.get_text("ADMIN_RW_NO_LIMIT", "No limit")
    )
...
async def manage_node(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if success:
        ...
        await callback.answer(
            texts.get_text("ADMIN_RW_NODE_ACTION_SUCCESS", "✅ Node {action}").format(
                action=action_texts.get(action, texts.get_text("ADMIN_RW_NODE_PROCESSED", "processed"))
            )
        )
    else:
        await callback.answer(
            texts.get_text("ADMIN_RW_NODE_ACTION_ERROR", "❌ Error performing action"), show_alert=True
        )
    ...
    try:
        ...
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.get_text("ADMIN_RW_BTN_REFRESH", "🔄 Refresh"),
                        callback_data=f"node_stats_{node_uuid}",
                    )
                ],
                [
                    types.InlineKeyboardButton(
                        text=texts.get_text("ADMIN_RW_BACK", "⬅️ Back"), callback_data=f"admin_node_manage_{node_uuid}"
                    )
                ],
            ]
        )
        ...
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=texts.get_text("ADMIN_RW_BTN_TRY_AGAIN", "🔄 Try again"),
                        callback_data=f"node_stats_{node_uuid}",
                    )
                ],
                [
...
async def show_squad_details(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if not squad:
        await callback.answer(texts.get_text("ADMIN_SQUAD_NOT_FOUND", "❌ Squad not found"), show_alert=True)
        return

    text = texts.get_text("ADMIN_SQUAD_DETAILS_HEADER", "🌐 <b>Squad: {name}</b>").format(name=squad["name"])
    ...
    text += "\n" + texts.get_text("ADMIN_SQUAD_DETAILS_MEMBERS", "- Members: {count}").format(
        count=squad["members_count"]
    )
    text += "\n" + texts.get_text("ADMIN_SQUAD_DETAILS_INBOUNDS_COUNT", "- Inbounds: {count}").format(
...
async def manage_squad_action(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if action == "add_users":
        ...
        if success:
            await callback.answer(texts.get_text("ADMIN_SQUAD_ADD_USERS_QUEUED", "✅ Task to add users queued"))
        else:
            await callback.answer(
                texts.get_text("ADMIN_SQUAD_ADD_USERS_ERROR", "❌ Error adding users"), show_alert=True
            )

    elif action == "remove_users":
        success = await remnawave_service.remove_all_users_from_squad(squad_uuid)
        if success:
            await callback.answer(texts.get_text("ADMIN_SQUAD_REMOVE_USERS_QUEUED", "✅ Task to remove users queued"))
        else:
            await callback.answer(
                texts.get_text("ADMIN_SQUAD_REMOVE_USERS_ERROR", "❌ Error removing users"), show_alert=True
            )
        ...
        if success:
            ...
            await callback.answer(
                texts.get_text("ADMIN_SQUAD_DELETE_ERROR", "❌ Error deleting squad"), show_alert=True
            )
        return
...
async def show_squad_edit_menu(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    text = texts.get_text("ADMIN_SQUAD_EDIT_HEADER", "✏️ <b>Editing squad: {name}</b>").format(name=squad["name"])
...
async def show_squad_inbounds_selection(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if not all_inbounds:
        await callback.answer(
            texts.get_text("ADMIN_SQUAD_NO_AVAILABLE_INBOUNDS", "❌ No available inbounds"), show_alert=True
        )
        return

    if squad_uuid not in squad_inbound_selections:
        squad_inbound_selections[squad_uuid] = set(inbound["uuid"] for inbound in squad.get("inbounds", []))
    ...
    text += "\n" + texts.get_text("ADMIN_SQUAD_CHANGE_INBOUNDS_CURRENT", "<b>Current inbounds:</b> {count}").format(
    ...
    if len(all_inbounds) > 15:
        text += "\n" + texts.get_text(
            "ADMIN_SQUAD_CHANGE_INBOUNDS_SHOWN", "⚠️ Showing first 15 of {total} inbounds"
    ...
    keyboard.extend(
        [
            ...
            [
                types.InlineKeyboardButton(
                    text=texts.get_text("ADMIN_SQUAD_BACK", "⬅️ Back"), callback_data=f"sqd_edit_{squad_uuid[:8]}"
                )
            ],
        ]
    )
...
async def show_squad_rename_form(callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    text += "\n\n" + texts.get_text("ADMIN_SQUAD_RENAME_CURRENT", "<b>Current name:</b> {name}").format(
        name=squad["name"]
    )
    ...
    text += "\n\n" + texts.get_text(
        "ADMIN_SQUAD_RENAME_INSTRUCTIONS", "Send a message with the new name or press 'Cancel' to exit."
    )

    keyboard = [
        [
            types.InlineKeyboardButton(
                text=texts.get_text("ADMIN_SQUAD_CANCEL", "❌ Cancel"), callback_data=f"cancel_rename_{squad_uuid}"
            )
        ]
    ]
...
async def process_squad_new_name(message: types.Message, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    if not squad_uuid:
        await message.answer(texts.get_text("ADMIN_SQUAD_RENAME_ERROR_NOT_FOUND", "❌ Error: squad not found"))
        await state.clear()
        return
    ...
    if not new_name:
        await message.answer(texts.get_text("ADMIN_SQUAD_RENAME_ERROR_EMPTY", "❌ Name cannot be empty. Try again:"))
        return
    ...
    if not re.match(r"^[A-Za-z0-9_-]+$", new_name):
        await message.answer(
            texts.get_text(
                "ADMIN_SQUAD_RENAME_ERROR_INVALID_CHARS",
    ...
    if new_name == old_name:
        await message.answer(
            texts.get_text(
                "ADMIN_SQUAD_RENAME_ERROR_SAME", "❌ New name is the same as current. Enter a different name:"
            )
        )
        return
...
async def toggle_squad_inbound(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if inbound_index >= len(all_inbounds):
        await callback.answer(texts.get_text("ADMIN_SQUAD_INBOUND_NOT_FOUND", "❌ Inbound not found"), show_alert=True)
        return
    ...
    keyboard.extend(
        [
            ...
            [
                types.InlineKeyboardButton(
                    text=texts.get_text("ADMIN_SQUAD_BACK", "⬅️ Back"), callback_data=f"sqd_edit_{short_squad_uuid}"
                )
            ],
        ]
    )
...
async def save_squad_inbounds(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    try:
        ...
        if success:
            ...
            await callback.answer(texts.get_text("ADMIN_SQUAD_CHANGES_SAVED", "✅ Changes saved!"))
        else:
            await callback.answer(texts.get_text("ADMIN_SQUAD_SAVE_ERROR", "❌ Error saving changes"), show_alert=True)

    except Exception as e:
        logger.error(f"Error saving squad inbounds: {e}")
        await callback.answer(
            texts.get_text("ADMIN_SQUAD_SAVE_ERROR_GENERIC", "❌ Error while saving"), show_alert=True
        )
...
async def start_squad_creation(callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    text += "\n" + texts.get_text("ADMIN_SQUAD_RENAME_REQ_NO_SPACES", "• No spaces or special characters")
    text += "\n\n" + texts.get_text(
        "ADMIN_SQUAD_CREATE_NAME_INSTRUCTIONS", "Send a message with the name or press 'Cancel' to exit."
    )

    keyboard = [
        [
            types.InlineKeyboardButton(
                text=texts.get_text("ADMIN_SQUAD_CANCEL", "❌ Cancel"), callback_data="cancel_squad_create"
            )
        ]
    ]
...
async def process_squad_name(message: types.Message, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    if not squad_name:
        await message.answer(texts.get_text("ADMIN_SQUAD_CREATE_ERROR_EMPTY", "❌ Name cannot be empty. Try again:"))
        return
    ...
    if not re.match(r"^[A-Za-z0-9_-]+$", squad_name):
        await message.answer(
            texts.get_text(
                "ADMIN_SQUAD_CREATE_ERROR_INVALID_CHARS",
                "❌ Name can only contain letters, numbers, hyphens and underscores. Try again:",
            )
        )
        return
    ...
    text = texts.get_text("ADMIN_SQUAD_CREATE_SELECT_INBOUNDS_HEADER", "➕ <b>Creating squad: {name}</b>").format(
        name=squad_name
    )
...
async def toggle_create_inbound(callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    if user_id not in squad_create_data:
        await callback.answer(
            texts.get_text("ADMIN_SQUAD_CREATE_ERROR_SESSION", "❌ Error: session data not found"), show_alert=True
        )
        await state.clear()
        return
...
async def finish_squad_creation(callback: types.CallbackQuery, db_user: User, db: AsyncSession, state: FSMContext):
    ...
    if success:
        await callback.message.edit_text(
            ...
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text=texts.get_text("ADMIN_SQUAD_LIST", "📋 Squad List"), callback_data="admin_rw_squads"
                        )
                    ],
                    [
                        types.InlineKeyboardButton(
        ...
        await callback.answer(texts.get_text("ADMIN_SQUAD_CREATED", "✅ Squad created!"))
    else:
        await callback.message.edit_text(
            texts.get_text(
        ...
        await callback.answer(
            texts.get_text("ADMIN_SQUAD_CREATE_ERROR_FAILED_SHORT", "❌ Squad creation error"), show_alert=True
        )
...
async def show_sync_options(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if status.last_run_finished_at:
        result_icon = "✅" if status.last_run_success else "❌"
        result_label = (
            texts.t("ADMIN_RW_SYNC_RESULT_SUCCESS", "success")
            if status.last_run_success
            else texts.t("ADMIN_RW_SYNC_RESULT_WITH_ERRORS", "with errors")
        )
        finished_text = format_datetime(status.last_run_finished_at)
        last_result = f"{result_icon} {result_label} ({finished_text})"
    elif status.last_run_started_at:
        last_result = texts.t("ADMIN_RW_SYNC_RESULT_STARTED", "⏳ Started at {started_at}").format(
            started_at=format_datetime(status.last_run_started_at)
        )

    status_lines = [
        texts.t("ADMIN_RW_SYNC_STATUS_LINE", "⚙️ Status: {status}").format(
            status=texts.t("ADMIN_RW_STATUS_ENABLED", "✅ Enabled")
            if status.enabled
            else texts.t("ADMIN_RW_STATUS_DISABLED", "❌ Disabled")
        ),
        texts.t("ADMIN_RW_SYNC_SCHEDULE_LINE", "🕒 Schedule: {schedule}").format(schedule=times_text),
        texts.t("ADMIN_RW_SYNC_NEXT_RUN_LINE", "📅 Next run: {next_run}").format(
            next_run=next_run_text if status.enabled else "—"
        ),
        texts.t("ADMIN_RW_SYNC_LAST_RUN_LINE", "📊 Last run: {last_result}").format(last_result=last_result),
    ]
...
async def toggle_auto_sync_setting(
    ...
    await callback.answer(
        texts.t("ADMIN_RW_AUTOSYNC_TOGGLED", "Auto-sync {status}").format(
            status=texts.t("ADMIN_RW_STATUS_ENABLED", "enabled")
            if new_value
            else texts.t("ADMIN_RW_STATUS_DISABLED", "disabled")
        )
...
async def run_auto_sync_now(
    ...
    if remnawave_sync_service.get_status().is_running:
        await callback.answer(
            texts.t("ADMIN_RW_SYNC_ALREADY_RUNNING_SHORT", "Sync is already running"), show_alert=True
        )
        return
...
async def save_auto_sync_schedule(
    ...
    if not parsed_times:
        await message.answer(
            texts.get_text(
                "ADMIN_RW_SYNC_SCHEDULE_PARSE_ERROR", "❌ Failed to parse time. Use HH:MM format, e.g. 03:00 or 18:45."
            ),
        )
        return
    ...
    keyboard.extend(
        [
            [
                ...
                types.InlineKeyboardButton(
                    text=texts.get_text("ADMIN_RW_NODES", "🌐 Nodes"), callback_data="admin_rw_nodes"
                ),
            ],
            [
                types.InlineKeyboardButton(
                    text=texts.get_text("ADMIN_RW_BACK", "⬅️ Back"), callback_data="admin_remnawave"
                )
            ],
        ]
    )
...
async def sync_users_to_panel(
    ...
    if stats["errors"] == 0:
        ...
        status_text = (
            texts.get_text("ADMIN_RW_SYNC_STATUS_WARNINGS", "completed with warnings")
            if status_emoji == "⚠️"
            else texts.get_text("ADMIN_RW_SYNC_STATUS_ERRORS", "completed with errors")
        )
    ...
    keyboard = [
        [
            types.InlineKeyboardButton(
                text=texts.get_text("ADMIN_RW_SYNC_RETRY", "🔄 Retry"), callback_data="sync_to_panel"
            )
        ],
        [
            types.InlineKeyboardButton(
                text=texts.get_text("ADMIN_RW_SYNC_FULL", "🔄 Full synchronization"), callback_data="sync_all_users"
            )
        ],
        [
            types.InlineKeyboardButton(
                text=texts.get_text("ADMIN_RW_SYNC_BACK", "⬅️ Back to sync"), callback_data="admin_rw_sync"
            )
        ],
    ]
...
async def show_sync_recommendations(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    await callback.message.edit_text(
        texts.get_text("ADMIN_RW_SYNC_ANALYZING", "🔍 Analyzing synchronization state..."), reply_markup=None
    )
...
async def cleanup_subscriptions(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if stats["deactivated"] > 0:
        ...
        text += "\n" + texts.get_text(
            "ADMIN_RW_SYNC_CLEANUP_ALL_CURRENT", "✅ All subscriptions are current!\nNo outdated subscriptions found."
        )

    if stats["errors"] > 0:
        text += "\n" + texts.get_text(
            "ADMIN_RW_SYNC_CLEANUP_ERRORS", "⚠️ Errors detected during processing.\nCheck logs for detailed information."
        )

    keyboard = [
        [
            types.InlineKeyboardButton(
                text=texts.get_text("ADMIN_RW_SYNC_RETRY_CLEANUP", "🔄 Retry cleanup"), callback_data="sync_cleanup"
            )
        ],
        [
            types.InlineKeyboardButton(
                text=texts.get_text("ADMIN_RW_SYNC_VALIDATION", "🔍 Validation"), callback_data="sync_validate"
            )
        ],
        [
            types.InlineKeyboardButton(
...
async def confirm_force_cleanup(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    keyboard = [
        ...
        [
            types.InlineKeyboardButton(
                text=texts.get_text("ADMIN_RW_CANCEL", "❌ Cancel"), callback_data="admin_rw_sync"
            )
        ],
    ]
...
async def sync_users(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if sync_type == "all_users":
        ...
        progress_text = texts.get_text(
            "ADMIN_RW_SYNC_USERS_PROGRESS", "🔄 Performing synchronization...\n\n⏳ Please wait..."
        )
    ...
    if sync_type == "all_users":
        ...
        text = texts.get_text(
            ...
            "{status_emoji} <b>Synchronization {status_text}</b>\n\n📊 <b>Result:</b>\n• ❌ Errors: {errors}",
        ).format(status_emoji=status_emoji, status_text=status_text, errors=stats["errors"])
    ...
    if stats["errors"] > 0:
        keyboard.append(
            [
                types.InlineKeyboardButton(
                    text=texts.get_text("ADMIN_RW_SYNC_RETRY", "🔄 Retry synchronization"), callback_data=callback.data
                )
            ]
        )
...
async def show_squads_management(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    keyboard.extend(
        [
            [
                types.InlineKeyboardButton(
                    text=texts.t("ADMIN_RW_SQUADS_CREATE", "➕ Create squad"), callback_data="admin_squad_create"
                )
            ],
            [
                types.InlineKeyboardButton(

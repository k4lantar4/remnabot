        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=texts.t("ADMIN_USER_MESSAGES_ADD", "📝 Add message"), callback_data="add_user_message"
                )
            ],
            [
                InlineKeyboardButton(
                    text=texts.t("ADMIN_USER_MESSAGES_LIST", "📋 Message list"), callback_data="list_user_messages:0"
                )
            ],
            [
                InlineKeyboardButton(
                    text=texts.t("ADMIN_USER_MESSAGES_STATS", "📊 Statistics"), callback_data="user_messages_stats"
                )
            ],
            [
                InlineKeyboardButton(
                    text=texts.t("ADMIN_USER_MESSAGES_BACK", "🔙 Back to admin"), callback_data="admin_panel"
                )
            ],
        ]
    )
...
def get_message_actions_keyboard(message_id: int, is_active: bool, language: str = "ru"):
    ...
    status_text = (
        texts.t("ADMIN_USER_MESSAGES_DEACTIVATE", "🔴 Deactivate")
        if is_active
        else texts.t("ADMIN_USER_MESSAGES_ACTIVATE", "🟢 Activate")
    )
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=texts.t("ADMIN_USER_MESSAGES_EDIT", "✏️ Edit"), callback_data=f"edit_user_message:{message_id}"
                )
            ],
...
async def process_new_message_text(message: types.Message, state: FSMContext, db_user: User, db: AsyncSession):
    ...
    try:
        ...
        active_status = (
            texts.t("ADMIN_USER_MESSAGES_ACTIVE", "🟢 Active")
            if new_message.is_active
            else texts.t("ADMIN_USER_MESSAGES_INACTIVE", "🔴 Inactive")
        )
        await message.answer(
            texts.t(
        ...
        await message.answer(
            texts.t(
                "ADMIN_USER_MESSAGES_CREATE_ERROR", "❌ An error occurred while creating the message. Please try again."
            ),
            reply_markup=get_user_messages_keyboard(db_user.language),
        )
...
async def list_user_messages(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if not messages:
        await callback.message.edit_text(
            texts.t(
                "ADMIN_USER_MESSAGES_LIST_EMPTY", "📋 <b>Message list</b>\n\nNo messages yet. Add the first message!"
            ),
            reply_markup=get_user_messages_keyboard(db_user.language),
            parse_mode="HTML",
        )
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                text=texts.t("ADMIN_USER_MESSAGES_NAV_BACK", "⬅️ Back"), callback_data=f"list_user_messages:{page - 1}"
            )
        )

    nav_buttons.append(
        InlineKeyboardButton(text=texts.t("ADMIN_USER_MESSAGES_NAV_ADD", "➕ Add"), callback_data="add_user_message")
    )

    if len(messages) == limit:
        nav_buttons.append(
            InlineKeyboardButton(
                text=texts.t("ADMIN_USER_MESSAGES_NAV_NEXT", "Next ➡️"), callback_data=f"list_user_messages:{page + 1}"
            )
        )
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                text=texts.t("ADMIN_USER_MESSAGES_NAV_BACK", "🔙 Back"), callback_data="user_messages_panel"
            )
        ]
    )
...
async def toggle_message_status(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    status_text = (
        texts.t("ADMIN_USER_MESSAGES_ACTIVATED", "activated")
        if message.is_active
        else texts.t("ADMIN_USER_MESSAGES_DEACTIVATED", "deactivated")
    )
    await callback.answer(
        texts.t("ADMIN_USER_MESSAGES_TOGGLE_SUCCESS", "✅ Message {status}").format(status=status_text)
    )

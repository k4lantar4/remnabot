    ...
    buttons = [
        [
            InlineKeyboardButton(
                text=texts.t("ADMIN_UPDATES_BTN_CHECK", "🔄 Check updates"), callback_data="admin_updates_check"
            )
        ],
        [
            InlineKeyboardButton(
                text=texts.t("ADMIN_UPDATES_BTN_INFO", "📋 Version info"), callback_data="admin_updates_info"
            )
        ],
        [
            InlineKeyboardButton(
...
def get_version_info_keyboard(language: str = "en") -> InlineKeyboardMarkup:
    ...
    buttons = [
        [
            InlineKeyboardButton(
                text=texts.t("ADMIN_UPDATES_BTN_REFRESH", "🔄 Refresh"), callback_data="admin_updates_info"
            )
        ],
        [InlineKeyboardButton(text=texts.t("ADMIN_UPDATES_BTN_BACK", "◀️ To updates"), callback_data="admin_updates")],
    ]
...
async def show_updates_menu(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    try:
        ...
        status_text = (
            texts.t("ADMIN_UPDATES_AVAILABLE", "{count} updates available").format(count=total_newer)
            if has_updates
            else texts.t("ADMIN_UPDATES_UP_TO_DATE", "Up to date")
        )

        last_check_text = ""
        if last_check:
            last_check_text = texts.t("ADMIN_UPDATES_LAST_CHECK", "\n🕐 Last check: {time}").format(

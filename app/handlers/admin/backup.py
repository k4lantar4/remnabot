        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=texts.t("ADMIN_BACKUP_BTN_CREATE", "🚀 Create backup"), callback_data="backup_create"
                ),
                InlineKeyboardButton(
                    text=texts.t("ADMIN_BACKUP_BTN_RESTORE", "📥 Restore"), callback_data="backup_restore"
                ),
            ],
            [
                InlineKeyboardButton(
                    text=texts.t("ADMIN_BACKUP_BTN_LIST", "📋 Backup list"), callback_data="backup_list"
                ),
                InlineKeyboardButton(
                    text=texts.t("ADMIN_BACKUP_BTN_SETTINGS", "⚙️ Settings"), callback_data="backup_settings"
                ),
            ],
...
def get_backup_list_keyboard(backups: list, page: int = 1, per_page: int = 5, language: str = "en"):
    ...
    keyboard.extend([[InlineKeyboardButton(text=texts.BACK, callback_data="backup_panel")]])
...
def get_backup_manage_keyboard(backup_filename: str, language: str = "en"):
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=texts.t("ADMIN_BACKUP_BTN_RESTORE", "📥 Restore"),
                    callback_data=f"backup_restore_file_{backup_filename}",
                )
            ],
            [
                InlineKeyboardButton(
                    text=texts.t("ADMIN_BACKUP_BTN_DELETE", "🗑️ Delete"),
...
def get_backup_settings_keyboard(settings_obj, language: str = "en"):
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text=texts.t("ADMIN_BACKUP_BTN_COMPRESSION", "🗜️ Compression: {status}").format(
...
async def create_backup_handler(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    success, message, file_path = await backup_service.create_backup(created_by=db_user.telegram_id, compress=True)
    ...
    if not backups:
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=texts.t("ADMIN_BACKUP_BTN_CREATE_FIRST", "🚀 Create first backup"),
        ...
        text = texts.t(
            "ADMIN_BACKUP_LIST_TITLE", "📦 <b>Backup list</b> (total: {count})\n\nSelect a backup to manage:"
...
async def delete_backup_confirm(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=texts.t("ADMIN_BACKUP_BTN_YES_DELETE", "✅ Yes, delete"),
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text=texts.t("ADMIN_BACKUP_BTN_TO_LIST", "📋 To backup list"), callback_data="backup_list"
                        )
                    ]
                ]
            ),
...
Continue with restore?""",
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=texts.t("ADMIN_BACKUP_BTN_YES_RESTORE", "✅ Yes, restore"),
                        callback_data=f"backup_restore_execute_{filename}",
                    ),
                    InlineKeyboardButton(
                        text=texts.t("ADMIN_BACKUP_BTN_CLEAR_RESTORE", "🗑️ Clear and restore"),
...
💡 Or select from existing backups below.""",
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=texts.t("ADMIN_BACKUP_BTN_SELECT_FROM_LIST", "📋 Select from list"),
...
async def restore_backup_execute(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    action_text = (
        texts.t("ADMIN_BACKUP_ACTION_CLEAR_RESTORE", "clearing and restoring")
        if clear_existing
        else texts.t("ADMIN_BACKUP_ACTION_RESTORE", "restoring")
    )
    progress_msg = await callback.message.edit_text(
        texts.t(
...
Continue?""",
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=texts.t("ADMIN_BACKUP_BTN_YES_RESTORE", "✅ Restore"),

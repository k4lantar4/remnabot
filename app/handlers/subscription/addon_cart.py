...
Шкала: после Phase C и ``price_kopeks`` корзины, и баланс — туманы 1:1, поэтому сравнение
идёт через ``user_can_afford``, а экран нехватки — общий для всех докупок
(``render_addon_insufficient_funds``), с кнопкой пополнения на недостающие туманы. Раньше
эти два числа были на разных шкалах, и прямое ``balance < price_kopeks`` из upstream давало
100-кратную ошибку.
"""

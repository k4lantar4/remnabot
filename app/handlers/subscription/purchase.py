        ...
        try:
            ...
            if tariff:
                ...
                if is_daily:
                    ...
                    if is_paused:
                        tariff_info_lines.append('')
                        tariff_info_lines.append(texts.t('DAILY_SUBSCRIPTION_PAUSED', '⏸️ Подписка приостановлена'))
                        ...
                        if last_charge:
                            ...
                            if next_charge > now:
                                ...
                                tariff_info_lines.append(texts.t('DAILY_CHARGE_PAUSED', '💤 Списание приостановлено'))
                    elif last_charge:
                        next_charge = last_charge + timedelta(hours=24)
                        now = datetime.now(UTC)
                        ...
                        tariff_info_lines.append('')
                        tariff_info_lines.append(texts.t('DAILY_FIRST_CHARGE_SOON', '⏳ Первое списание скоро'))
...
📱 Устройства: {devices_used} / {device_limit}""",
    ...
    if not show_devices:
        devices_line_pattern = texts.t(
            'SUBSCRIPTION_OVERVIEW_DEVICES_LINE',
            '\n📱 Устройства: {devices_used} / {device_limit}',
        )
        message_template = message_template.replace(devices_line_pattern, '')
...
async def activate_trial(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    try:
        ...
        if remnawave_user and subscription_link:
            ...
            connect_keyboard = await _build_trial_success_keyboard(
                texts,
                subscription,
                subscription.id,
                subscription_link,
            ...
            ).format(activated=texts.TRIAL_ACTIVATED)
            trial_success_text += payment_note
            await callback.message.edit_text(
                trial_success_text,
                reply_markup=get_back_keyboard(db_user.language),
...
async def return_to_saved_cart(callback: types.CallbackQuery, state: FSMContext, db_user: User, db: AsyncSession):
    ...
    if 'period_days' not in prepared_cart_data:
        await callback.answer(
            texts.t('CB_CART_CORRUPTED', '❌ Корзина повреждена. Оформите подписку заново.'), show_alert=True
        )
    ...
    if total_price > 0 and not user_can_afford(db_user.balance_kopeks, total_price):
        ...
        insufficient_text = texts.t(
            ...
            balance=texts.format_balance(db_user.balance_kopeks, round_kopeks=False),
            missing=texts.format_balance(missing_amount, round_kopeks=False),
        )
    ...
    summary_lines = [
        texts.t('PURCHASE_CART_RESTORED_TITLE', '🛒 Восстановленная корзина'),
        '',
        texts.t('PURCHASE_CART_PERIOD_LINE', '📅 Период: {period}').format(period=period_display),
        texts.t('PURCHASE_CART_TRAFFIC_LINE', '📊 Трафик: {traffic}').format(traffic=traffic_display),
        texts.t('PURCHASE_CART_COUNTRIES_LINE', '🌍 Страны: {countries}').format(
    ...
    if settings.is_devices_selection_enabled():
        ...
        if devices_value is not None:
            summary_lines.append(
                texts.t('PURCHASE_CART_DEVICES_LINE', '📱 Устройства: {devices}').format(devices=devices_value)
            )

    summary_lines.extend(
        [
            '',
            texts.t('PURCHASE_CART_TOTAL_LINE', '💎 Общая стоимость: {price}').format(
                price=texts.format_price(total_price)
            ),
            '',
            texts.t('PURCHASE_CART_CONFIRM_PROMPT', 'Подтверждаете покупку?'),
        ]
    )
...
async def handle_extend_subscription(
    ...
    trial_ending_text = texts.t(
        'TRIAL_PERIOD_ENDING_SELECT_TARIFF',
        '🎯 <b>Пробный период заканчивается</b>\n\nЧтобы продолжить пользоваться VPN, выберите подходящий тариф.',
    )
    trial_ending_keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
    ...
    if not subscription:
        await callback.message.edit_text(
            trial_ending_text,
            reply_markup=trial_ending_keyboard,
            parse_mode='HTML',
        )
        await callback.answer()
        return
    ...
    if not renewal_prices:
        await callback.answer(
            texts.t('CB_NO_RENEWAL_PERIODS', '⚠ Нет доступных периодов для продления'), show_alert=True
        )
        return
    ...
    renewal_lines = [
        texts.t('SUBSCRIPTION_RENEWAL_TITLE', '⏰ Продление подписки'),
        '',
        texts.t('SUBSCRIPTION_RENEWAL_DAYS_LEFT', 'Осталось дней: {days}').format(days=subscription.days_left),
        '',
        texts.t('SUBSCRIPTION_RENEWAL_CURRENT_CONFIG', '<b>Ваша текущая конфигурация:</b>'),
        texts.t('SUBSCRIPTION_RENEWAL_SERVERS_COUNT', '🌍 Серверов: {count}').format(
            count=len(subscription.connected_squads or [])
        ),
        texts.t('SUBSCRIPTION_RENEWAL_TRAFFIC_LINE', '📊 Трафик: {traffic}').format(
    ...
    if settings.is_devices_selection_enabled():
        renewal_lines.append(
            texts.t('SUBSCRIPTION_RENEWAL_DEVICES_LINE', '📱 Устройств: {devices}').format(
    ...
    renewal_lines.extend(
        [
            '',
            texts.t('SUBSCRIPTION_RENEWAL_SELECT_PERIOD', '<b>Выберите период продления:</b>'),
            prices_text.rstrip(),
            '',
        ]
    )
    ...
    message_text += texts.t(
        'SUBSCRIPTION_RENEWAL_PRICE_NOTE',
...
async def confirm_extend_subscription(
    ...
    if settings.is_multi_tariff_enabled():
        ...
        if _fsm_sub_id:
            ...
            await callback.answer(
                texts.t('CB_SELECT_SUBSCRIPTION_VIA_MY_SUBS', 'Выберите подписку через "Мои подписки"'), show_alert=True
            )
            return
    else:
        subscription = db_user.subscription
    ...
    if settings.is_traffic_fixed() and result.subscription.traffic_limit_gb != old_traffic_gb:
        ...
        success_message += texts.t(
            'SUBSCRIPTION_RENEWAL_TRAFFIC_RESET',
            '\n\n📊 Трафик сброшен до {limit} ГБ',
        ).format(limit=fixed_limit)

    if promo_offer_discount > 0:
        success_message += texts.t(
            'SUBSCRIPTION_RENEWAL_PROMO_DISCOUNT',
...
async def select_period(callback: types.CallbackQuery, state: FSMContext, db_user: User, db: AsyncSession):
    ...
    if settings.is_traffic_selectable():
        ...
        if not available_packages:
            await callback.answer(
                texts.t('TRAFFIC_PACKAGES_NOT_CONFIGURED', '⚠️ Пакеты трафика не настроены'), show_alert=True
            )
            return
...
async def confirm_purchase(callback: types.CallbackQuery, state: FSMContext, db_user: User, db: AsyncSession):
    ...
    if price_difference > 0:
        ...
        if price_difference > max_allowed_increase:
            ...
            await callback.answer(
                texts.t('CB_PRICE_CHANGED_RESTART', 'Цена изменилась. Пожалуйста, начните оформление заново.'),
                show_alert=True,
            )
            return
    ...
    try:
        ...
        if remnawave_user and subscription_link:
            ...
            connect_keyboard = await build_post_purchase_connect_keyboard(texts, subscription)
...
async def handle_subscription_settings(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if settings.is_multi_tariff_enabled():
        await callback.answer(
            texts.t('CB_SETTINGS_VIA_MY_SUBS', 'Настройки доступны через "Мои подписки"'), show_alert=True
        )
        return
    ...
    if not show_devices:
        settings_devices_line = texts.t(
            'SUBSCRIPTION_SETTINGS_DEVICES_LINE',
            '\n📱 Устройства: {devices_used} / {devices_limit}',
        )
        settings_template = settings_template.replace(settings_devices_line, '')
...
async def _build_trial_success_keyboard(
    texts,
    subscription,
    sub_id: int,
    ...
    if connect_mode == 'miniapp_subscription':
        return await build_miniapp_subscription_connect_keyboard(
            subscription=subscription,
            sub_id=sub_id,
            texts=texts,
            back_rows=back_rows,
    ...
    try:
        ...
        if payment_method == 'stars':
            ...
            if not payment_result or not payment_result.get('confirmation_url'):
                await callback.answer(
                    texts.t('CB_PAYMENT_CREATE_FAILED', '❌ Не удалось создать платеж. Попробуйте позже.'),
                    show_alert=True,
                )
                return
        ...
        await callback.answer(
            texts.t('CB_PAYMENT_CREATE_ERROR_GENERIC', '❌ Произошла ошибка при создании платежа. Попробуйте позже.'),
            show_alert=True,
        )
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        handle_subscription_link,
        F.data.startswith('sl:') | F.data.startswith('sl_self:') | F.data.startswith('sl_share:') | F.data.startswith('sl_config:'),
    )
...
async def handle_simple_subscription_purchase(
    ...
    simple_lines.extend(
        [
            texts.t('SIMPLE_SUBSCRIPTION_TRAFFIC', '📊 Трафик: {traffic}').format(traffic=traffic_text),
    ...
    if user_can_afford(user_balance_kopeks, price_kopeks):
        ...
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                [
                    types.InlineKeyboardButton(
                        text=texts.t('SIMPLE_SUBSCRIPTION_OTHER_PAYMENT_BTN', '💳 Другие способы оплаты'),
        ...
        simple_lines.append(texts.t('SIMPLE_SUBSCRIPTION_SELECT_PAYMENT', 'Выберите способ оплаты:'))
        message_text = '\n'.join(simple_lines)
...
async def _extend_existing_subscription(
    ...
    if current_subscription.is_trial:
        success_message += texts.t(
            'SUBSCRIPTION_RENEWAL_TRIAL_CONVERTED',

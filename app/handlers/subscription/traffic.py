...
async def handle_reset_traffic(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if settings.is_traffic_fixed():
        await callback.answer(
            texts.t("TRAFFIC_FIXED_MODE", "⚠️ In current mode traffic is fixed and cannot be reset"), show_alert=True
        )
        return
    ...
    if not subscription or subscription.is_trial:
        await callback.answer(
            texts.t("PAID_FEATURE_ONLY", "⚠️ This feature is only available for paid subscriptions"), show_alert=True
        )
        return

    if subscription.traffic_limit_gb == 0:
        await callback.answer(
            texts.t("TRAFFIC_ALREADY_UNLIMITED", "⚠️ You already have unlimited traffic"), show_alert=True
        )
        return
    ...
    if db_user.balance_toman < reset_price:
        await callback.answer(
            texts.t("subscription.traffic.reset.insufficient_balance", "⌛ Insufficient balance"), show_alert=True
        )
        return
    ...
    await callback.message.edit_text(
        reset_text, reply_markup=get_reset_traffic_confirm_keyboard(reset_price, db_user.language)
    )
...
async def confirm_reset_traffic(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if settings.is_traffic_fixed():
        await callback.answer(texts.t("TRAFFIC_FIXED_MODE", "⚠️ In current mode traffic is fixed"), show_alert=True)
        return
    ...
    try:
        success = await subtract_user_balance(
            db, db_user, reset_price, texts.t("subscription.traffic.reset.transaction_desc", "Traffic reset")
        )
    ...
    try:
        ...
        for pkg in packages:
            if pkg["enabled"]:
                ...
                logger.info(f"   📦 {gb_text}: {pkg['price'] / 100} Toman")
    ...
    try:
        ...
        if enabled_packages:
            ...
            for pkg in enabled_packages:
                gb_text = "♾️ Unlimited" if pkg["gb"] == 0 else f"{pkg['gb']} GB"
                info_lines.append(f"   • {gb_text}: {pkg['price'] // 100} Toman")
...
async def add_traffic(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    try:
        ...
        await create_transaction(
            ...
            type=TransactionType.SUBSCRIPTION_PAYMENT,
            amount_toman=price,
            description=texts.t("subscription.traffic.add_transaction_desc", "Adding {gb} GB traffic").format(
                gb=traffic_gb
            ),
        )
        ...
        if traffic_gb == 0:
            ...
            success_text = texts.t(
                ...
                ("✅ Traffic successfully added!\n\n📈 Added: {gb} GB\nNew limit: {limit}"),
            ).format(gb=traffic_gb, limit=texts.format_traffic(subscription.traffic_limit_gb))
        ...
        if price > 0:
            ...
            if total_discount_value > 0:
                success_text += texts.t(
                    "subscription.traffic.add.discount_info", " (discount {percent}%: -{amount})"
                ).format(percent=discount_result["percent"], amount=texts.format_price(total_discount_value))
...
async def confirm_switch_traffic(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    if new_traffic_gb == current_traffic:
        await callback.answer(texts.t("TRAFFIC_NO_CHANGE", "ℹ️ Traffic limit unchanged"), show_alert=True)
        return
    ...
    if price_difference_per_month > 0:
        ...
        if db_user.balance_toman < total_price_difference:
            ...
            message_text = texts.t(
                ...
                required=texts.t("subscription.countries.charged_period", "{amount} (for {months} months)").format(
                    amount=texts.format_price(total_price_difference), months=months_remaining
                ),
                balance=texts.format_price(db_user.balance_toman),
                missing=texts.format_price(missing_toman),
            )
        ...
        cost_text = texts.t(
            "subscription.traffic.switch.additional_payment", "Additional payment: {amount} (for {months} months)"
        ).format(amount=texts.format_price(total_price_difference), months=months_remaining)
        if discount_savings_per_month > 0:
            total_discount_savings = discount_savings_per_month * months_remaining
            cost_text += texts.t(
                "subscription.traffic.switch.discount_info", " (discount {percent}%: -{amount})"
    ...
    try:
        if price_difference > 0:
            success = await subtract_user_balance(
                ...
                price_difference,
                texts.t(
                    "subscription.traffic.switch.transaction_desc", "Switching traffic from {old}GB to {new}GB"
            ...
            await create_transaction(
                ...
                description=texts.t(
                    "subscription.traffic.switch.transaction_desc_full",
                    "Switching traffic from {old}GB to {new}GB for {months} months",
                ).format(old=current_traffic, new=new_traffic_gb, months=months_remaining),
            )
        ...
        logger.info(
            f"✅ User {db_user.telegram_id} switched traffic from {current_traffic}GB to {new_traffic_gb}GB, additional payment: {price_difference / 100} Toman"
        )

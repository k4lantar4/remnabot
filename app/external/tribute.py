...
class TributeService:
    ...
    async def create_payment_link(
        self, user_id: int, amount_toman: int = 0, description: str = "Balance top-up"
    ) -> Optional[str]:
    ...
    async def process_webhook(self, payload_or_data) -> Optional[Dict[str, Any]]:
        try:
            ...
            logger.info(
                f"Extracted data: payment_id={payment_id}, status={status}, amount_toman={amount_toman}, user_id={telegram_user_id}"
            )
    ...
    async def refund_payment(
        self, payment_id: str, amount_toman: Optional[int] = None, reason: str = "Refund on request"

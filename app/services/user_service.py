...
class UserService:
    async def _send_balance_notification(self, bot: Bot, user: User, amount_toman: int, admin_name: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                if mulenpay_payments:
                    ...
                    logger.info(f"Deleting {len(mulenpay_payments)} {mulenpay_name} payments")
                    await db.execute(
                ...
                logger.error(f"Error deleting {settings.get_mulenpay_display_name()} payments: {e}")
            ...
            try:
                ...
                if heleket_payments:
                    logger.info(f"Deleting {len(heleket_payments)} Heleket payments")
                    await db.execute(

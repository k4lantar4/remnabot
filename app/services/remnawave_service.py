...
def _get_user_traffic_bytes(panel_user: Dict[str, Any]) -> int:
    """Extracts usedTrafficBytes from a panel user (compatible with new and old API)."""
    # New format: userTraffic.usedTrafficBytes
    ...
    # Old format: usedTrafficBytes directly on the user
    return panel_user.get("usedTrafficBytes", 0)


def _get_lifetime_traffic_bytes(panel_user: Dict[str, Any]) -> int:
    """Extracts lifetimeUsedTrafficBytes from a panel user (compatible with new and old API)."""
    # New format: userTraffic.lifetimeUsedTrafficBytes
    user_traffic = panel_user.get("userTraffic")
    if user_traffic and isinstance(user_traffic, dict):
        return user_traffic.get("lifetimeUsedTrafficBytes", 0)
    # Old format: lifetimeUsedTrafficBytes directly on the user
...
class RemnaWaveService:
    ...
    def _ensure_configured(self) -> None:
        if not self.is_configured or self.api is None:
            raise RemnaWaveConfigurationError(self._config_error or "RemnaWave API is not configured")
    ...
    def _parse_remnawave_date(self, date_str: str) -> datetime:
        ...
        try:
            ...
            logger.debug(f"Successfully parsed date: {date_str} -> {utc_normalized} (normalized to UTC)")
            return utc_normalized

        except Exception as e:
            logger.warning(f"⚠️ Failed to parse date '{date_str}': {e}. Using default date.")
            return self._now_utc() + timedelta(days=30)
    ...
    async def get_system_statistics(self) -> Dict[str, Any]:
        try:
            async with self.get_api_client() as api:
                ...
                logger.info(
                    f"Statistics compiled: users={result['system']['total_users']}, total traffic={total_user_traffic}"
                )
                return result
        ...
        try:
                    ...
                    if not db_user:
                        if sync_type in ["new_only", "all"]:
                            ...
                            if is_created:
                                ...
                                stats["updated"] += 1
                                logger.info(f"♻️ Updated subscription for existing user {telegram_id}")
            ...
            if sync_type == "all":
                ...
                for telegram_id, db_user in bot_users_by_telegram_id.items():
                    if (
                        telegram_id not in panel_telegram_ids
                        ...
                        try:
                            ...
                            if processed_count % batch_size == 0:
                                try:
                                    await db.commit()
                                    logger.debug(
                                        f"📦 Committed changes after deactivating {processed_count} subscriptions"
                                    )
                                    cleanup_uuid_mutations.clear()
                                except Exception as commit_error:
                                    logger.error(
                                        f"❌ Commit error after deactivating {processed_count} subscriptions: {commit_error}"
                                    )
                                    await db.rollback()
                                    for mutation in reversed(cleanup_uuid_mutations):
                                        mutation.rollback()
            ...
            logger.info(
                f"🎯 Synchronization completed: created {stats['created']}, updated {stats['updated']}, deactivated {stats['deleted']}, errors {stats['errors']}"
            )
            return stats
    ...
    async def test_api_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "message": f"Connection error: {str(e)}", "api_url": settings.REMNAWAVE_API_URL}
    ...
    async def get_node_user_usage_by_range(self, node_uuid: str, start_date, end_date) -> List[Dict[str, Any]]:
        try:
            async with self.get_api_client() as api:
                ...
                usage_data = await api._make_request("GET", f"/api/nodes/usage/{node_uuid}/users/range", params=params)
    ...
    async def cleanup_orphaned_subscriptions(self, db: AsyncSession) -> Dict[str, int]:
        try:
                ...
                for subscription in subscriptions:
                    try:
                        ...
                        if user.telegram_id not in panel_telegram_ids:
                            logger.info(
                                f"🗑️ FULL deactivation of subscription for user {user.telegram_id} (not in panel)"
                            )
            ...
            logger.info(
                f"🧹 Enhanced cleanup completed: checked {stats['checked']}, deactivated {stats['deactivated']}, errors {stats['errors']}"
            )
            return stats
    ...
    async def sync_subscription_statuses(self, db: AsyncSession) -> Dict[str, int]:
        try:
            ...
            logger.info(
                f"🔄 Status synchronization completed: checked {stats['checked']}, updated {stats['updated']}, errors {stats['errors']}"
            )
            return stats
    ...
    async def validate_and_fix_subscriptions(self, db: AsyncSession) -> Dict[str, int]:
        try:
            ...
            logger.info(
                f"🔍 Validation completed: checked {stats['checked']}, fixed subscriptions {stats['fixed']}, issues found {stats['issues_found']}, errors {stats['errors']}"
            )
            return stats
        ...
        try:
            ...
            if new_status == "offline":
                ...
                if status_data.get("nodes_health") == "unhealthy":
                    issues.append(
                        f"Node issues ({status_data.get('nodes_online')}/{status_data.get('total_nodes')} online)"
                    )

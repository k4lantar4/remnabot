...
class TenantFeatureService:
    ...
    @staticmethod
    async def is_feature_enabled(db: AsyncSession, bot_id: int, feature_key: str, use_cache: bool = True) -> bool:
        """
        ...
        if use_cache:
            try:
                ...
                if cached is not None:
                    logger.debug(f"✅ Cache hit for feature flag: {feature_key} (bot_id={bot_id})")
                    return bool(cached)
            except Exception as e:
                logger.warning(f"⚠️ Cache read error for {cache_key_str}: {e}, falling back to DB")
        ...
        if use_cache:
            try:
                ...
                logger.debug(f"💾 Cached feature flag: {feature_key} (bot_id={bot_id}) = {enabled}")
            except Exception as e:
                logger.warning(f"⚠️ Cache write error for {cache_key_str}: {e}")

        return enabled

    @staticmethod
    async def get_feature_config(
        db: AsyncSession, bot_id: int, feature_key: str, use_cache: bool = True
        ...
        """
        cache_key_str = cache_key(TenantFeatureService.CACHE_PREFIX, bot_id, feature_key, "config")

        if use_cache:
            try:
                cached = await cache.get(cache_key_str)
                if cached is not None:
                    logger.debug(f"✅ Cache hit for feature config: {feature_key} (bot_id={bot_id})")
        ...
        if use_cache and config:
            try:
                await cache.set(cache_key_str, config, expire=TenantFeatureService.CACHE_TTL)
                logger.debug(f"💾 Cached feature config: {feature_key} (bot_id={bot_id})")
            except Exception as e:
                logger.warning(f"⚠️ Cache write error for {cache_key_str}: {e}")
    ...
    async def set_feature(
        ...
        try:
            cache_key_str = TenantFeatureService._get_cache_key(bot_id, feature_key)
            config_cache_key = cache_key(TenantFeatureService.CACHE_PREFIX, bot_id, feature_key, "config")

            await cache.delete(cache_key_str)
            await cache.delete(config_cache_key)

            logger.info(
                f"🔄 Feature flag updated and cache invalidated: {feature_key} (bot_id={bot_id}, enabled={enabled})"
            )
        except Exception as e:
            logger.warning(f"⚠️ Cache invalidation error for {feature_key} (bot_id={bot_id}): {e}")

    @staticmethod
    async def get_all_features(db: AsyncSession, bot_id: int, enabled_only: bool = False) -> Dict[str, bool]:
        """
    ...
    @staticmethod
    async def invalidate_cache(bot_id: int, feature_key: Optional[str] = None) -> None:
        """
        ...
        try:
            if feature_key:
                ...
                await cache.delete(cache_key_str)
                await cache.delete(config_cache_key)
                logger.info(f"🗑️ Cache invalidated for feature: {feature_key} (bot_id={bot_id})")
            else:
            ...
            logger.error(f"❌ Error invalidating cache for bot_id={bot_id}, feature_key={feature_key}: {e}")

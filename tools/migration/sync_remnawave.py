...
def _load_remaining_bytes_map(manifest_path: Path | None = None) -> dict[tuple[int, int], int]:
    ...
    for row in data.get('subscribers', []):
        ...
        out[(tg, tariff_id)] = int(row.get('remaining_bytes') or 0)
    return out
...
async def sync_all(
    ...
    *,
    ...
    for sub, user in rows:
        key = (user.telegram_id, sub.tariff_id) if sub.tariff_id else (user.telegram_id, 0)

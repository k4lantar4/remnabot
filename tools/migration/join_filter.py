...
def build_migration_cohorts(
    ...
    *,
    ...
    for (tg_id, email), members in sorted(groups.items()):
        if not members:
            continue
        m = members[0]
        user = m['user']
        exp = m['expiry_unix']
        if exp <= 0:
            end_date = migration_run + timedelta(days=3650)
        else:
            end_date = compute_end_date(exp, migration_run)
        ...
        subscribers.append(
            MigrationSubscription(
                ...
                squad_keys=[m['squad_key']],
                end_date=end_date,
                remaining_bytes=m['remaining_bytes'],
                traffic_limit_gb=m['traffic_limit_gb'],
                old_emails=[email],
                old_uuids=[m['client'].uuid],
                source_email=email,
            )
        )

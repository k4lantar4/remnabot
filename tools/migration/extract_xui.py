...
def _discover_server_ids(primary: Path, fallback: Path) -> set[int]:
    server_ids: set[int] = set()
    for backup_dir in (primary, fallback):
        if not backup_dir.is_dir():
            continue
        for db_path in backup_dir.glob('server*.db'):
            try:
                server_ids.add(_parse_server_id(db_path))
            except ValueError:
                continue
    return server_ids
...
def _candidate_db_paths(
    server_id: int,
    *,
    primary: Path = XUI_BACKUP_DIR_PRIMARY,
    fallback: Path = XUI_BACKUP_DIR_FALLBACK,
) -> list[Path]:
    """Ordered backup candidates: evening primary, then morning fallback variants."""
    ordered: list[Path] = []
    seen: set[Path] = set()
    for backup_dir, suffix in ((primary, '_19-00'), (fallback, '_08-00'), (fallback, '_08-01')):
        if not backup_dir.is_dir():
            continue
        paths = [backup_dir / f'server{server_id}-2026-02-28{suffix}.db']
        paths.extend(sorted(backup_dir.glob(f'server{server_id}-*.db')))
        for path in paths:
            if path in seen:
                continue
            seen.add(path)
...
def resolve_server_db_path(
    server_id: int,
    *,
    primary: Path = XUI_BACKUP_DIR_PRIMARY,
    fallback: Path = XUI_BACKUP_DIR_FALLBACK,
) -> Path | None:
    """First non-empty backup file: evening primary, morning fallback second."""
    candidates = _candidate_db_paths(server_id, primary=primary, fallback=fallback)
    return candidates[0] if candidates else None


def extract_all_xui_clients(
    primary: Path = XUI_BACKUP_DIR_PRIMARY,
    fallback: Path = XUI_BACKUP_DIR_FALLBACK,
) -> list[XuiClient]:
    all_clients: list[XuiClient] = []
    for server_id in sorted(_discover_server_ids(primary, fallback)):
        if server_id in SKIP_SERVER_IDS:
            continue
        vip = SERVER_VIP.get(server_id, 20)
        for db_path in _candidate_db_paths(server_id, primary=primary, fallback=fallback):
            try:
                clients = extract_clients_from_db(db_path.resolve(), server_id, vip)

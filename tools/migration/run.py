...
from tools.migration.purge import purge_bot_users, purge_panel_users
...
async def cmd_validate(manifest_path: Path | None = None) -> dict:
    if manifest_path is None:
        manifest_path = MIGRATION_OUTPUT_DIR / 'migration_manifest.json'
    async with get_db_session() as db:
        report = await validate_migration(db, manifest_path=manifest_path)
    print(json.dumps(report, indent=2))
    return report


async def cmd_purge(*, execute: bool, panel: bool, bot: bool) -> dict:
    if execute:
        require_backup_gate()

    result: dict = {'dry_run': not execute}
    if panel:
        result['panel'] = await purge_panel_users(dry_run=not execute)
    if bot:
        async with get_db_session() as db:
            result['bot'] = await purge_bot_users(db, dry_run=not execute)
...
def main() -> None:
    ...
    purge_p = sub.add_parser('purge', help='Remove all VPN users from panel and/or bot DB')
    purge_p.add_argument('--execute', action='store_true', help='Requires MIGRATION_BACKUP_DIR')
    purge_p.add_argument('--panel', action='store_true', help='Delete Remnawave panel users only')
    purge_p.add_argument('--bot', action='store_true', help='Truncate bot users/subscriptions')

    validate_p = sub.add_parser('validate', help='Post-migration validation checks')
    validate_p.add_argument(
        '-m',
        '--manifest',
        type=Path,
        default=MIGRATION_OUTPUT_DIR / 'migration_manifest.json',
        help='Manifest from extract for expected user counts',
    )
    ...
    if args.cmd == 'backup':
        ...
        if not args.panel and not args.bot:
            parser.error('purge requires at least one of --panel or --bot')
        asyncio.run(cmd_purge(execute=args.execute, panel=args.panel, bot=args.bot))
    elif args.cmd == 'validate':
        asyncio.run(cmd_validate(args.manifest))

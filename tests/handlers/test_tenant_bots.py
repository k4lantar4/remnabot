...
def mock_callback():
    ...
    callback.from_user.id = 12345  # Admin user ID
...
class TestListTenantBots:
    ...
    async def test_list_tenant_bots_displays_all_required_columns(
        ...
        # Mock multiple database queries (main query fails, fallback query succeeds, count query)
        # Main query fails (tenant_subscriptions table doesn't exist), so Exception is raised
        mock_db.execute.side_effect = [
            Exception("Table tenant_subscriptions does not exist"),  # Main query fails
            MagicMock(all=MagicMock(return_value=[(mock_bot, 10, 50000)])),  # Fallback query succeeds
            MagicMock(scalar=MagicMock(return_value=1))  # Count query
        ]

        # Mock get_bot_by_id and admin check
        with patch('app.database.crud.bot.get_bot_by_id', return_value=mock_bot), \
             patch('app.localization.texts.get_texts') as mock_get_texts, \
             patch('app.utils.decorators.settings') as mock_settings:
            # Set up mock bot attributes
    ...
    async def test_list_tenant_bots_pagination_works(
        ...
        with patch('app.database.crud.bot.get_bot_by_id', return_value=mock_bot), \
             patch('app.localization.texts.get_texts') as mock_get_texts, \
             patch('app.keyboards.admin.get_admin_pagination_keyboard') as mock_pagination:
            mock_texts = MagicMock()
            mock_texts.t = lambda key, default: default
            mock_get_texts.return_value = mock_texts

...
Жалоба (upstream 0009c30b): докупил трафик, денег не хватило, пополнил — «трафик
не покупается сам», а «Вернуться к оформлению подписки» отвечает «Корзина
повреждена» и удаляет корзину; следующее нажатие — «Корзина не найдена».
...
def _user(balance_toman: int) -> SimpleNamespace:
    return SimpleNamespace(id=7, telegram_id=1, language='ru', balance_kopeks=balance_toman)
...
async def test_button_resumes_addon_cart_instead_of_corrupting_it():
    ...
    with (
        ...
        await return_to_saved_cart(callback, AsyncMock(), _user(balance_toman=10000), AsyncMock())
...
async def test_button_with_still_insufficient_balance_sends_back_to_topup():
    ...
    with patch('app.handlers.subscription.addon_cart.resume_addon_cart', resumed):
        # 10 томанов на балансе, корзина стоит 50 томанов.
        await resume_addon_cart_from_button(callback, _user(balance_toman=10), AsyncMock(), _traffic_cart())

    resumed.assert_not_called()
    text = callback.message.edit_text.call_args.args[0]
    assert 'недостаточно' in text.lower()
    assert '40' in text, 'не хватает 40 томанов (50 − 10)'
...
async def test_button_compares_toman_balance_with_catalog_price():
    ...
    with patch('app.handlers.subscription.addon_cart.resume_addon_cart', resumed):
        await resume_addon_cart_from_button(callback, _user(balance_toman=60), AsyncMock(), _traffic_cart())
...
async def test_button_reports_failure_without_deleting_cart():
    ...
    with patch('app.handlers.subscription.addon_cart.resume_addon_cart', AsyncMock(return_value=False)):
        await resume_addon_cart_from_button(callback, _user(balance_toman=10000), AsyncMock(), _traffic_cart())
...
# ---------------------------------------------------------------------------
# Ручной запуск: тот же диспетчер, помеченный как ручной
# ---------------------------------------------------------------------------

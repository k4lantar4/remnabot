...
USER_ID = 7
SUB_ID = 42
TRAFFIC_PRICE = 5000  # каталог: 50 томанов за пакет в месяц
DEVICE_PRICE = 1000  # каталог: 10 томанов за устройство в месяц
BALANCE = 100_000  # томаны
...
async def _seed(db, *, balance: int = BALANCE) -> tuple[User, Subscription]:
...
async def test_topup_really_buys_devices_from_saved_cart(monkeypatch, cart_service):
    ...
    charged_toman = BALANCE - after.balance
...
async def test_partial_topup_keeps_cart_and_intent(monkeypatch, cart_service):
    async with memory_session(monkeypatch, list(Base.metadata.sorted_tables)) as db:
        # 10 томанов — меньше пропорциональной цены пакета (около 25 томанов).
        user, _ = await _seed(db, balance=10)
        await cart_service.save_user_cart(USER_ID, _traffic_cart())
    ...
    assert after.balance == 10 and after.traffic == 100 and after.payments == []
...
def test_every_payment_provider_calls_the_shared_topup_hook():
    ...
    У нас сверх upstream — карта-на-карту (плагин C2C, единственный живой способ) и
    ручное пополнение админом.
    """
    from pathlib import Path

    app_dir = Path(__file__).resolve().parents[2] / 'app'
    payment_dir = app_dir / 'services' / 'payment'
    ...
    extra = [
        app_dir / 'services' / 'tribute_service.py',
        app_dir / 'services' / 'apple_iap.py',
        app_dir / 'services' / 'manual_topup_service.py',
        app_dir / 'plugins' / 'c2c' / 'service.py',
    ]
